...
public:
	...
	ChatCommand* GetCommands() const 
	{
		...
		{ 
			...
			{ "weapon", 	SEC_ADMINISTRATOR, 	false, &HandleNpcAddWeaponCommand, "", NULL }, //TODO: fix or remove this command
		...
		{ 
			...
			{ "name", 			SEC_GAMEMASTER, 	false, &HandleNpcSetNameCommand, "", NULL },  //{ TODO: fix or remove these commands
			{ "subname", 		SEC_GAMEMASTER, 	false, &HandleNpcSetSubNameCommand, "", NULL }, //{ TODO: fix or remove these commands
			{ NULL, 0, false, NULL, "", NULL } 
		};
	...
	static bool HandleNpcPlayEmoteCommand(ChatHandler* handler, const char* args) 
	{
		...
		if (target->GetTransport())
			if (target->GetGUIDTransport())
				WorldDatabase.PQuery( "UPDATE creature_transport SET emote=%u WHERE transport_entry=%u AND guid=%u",
										emote, target->GetTransport()->GetEntry(), target->GetGUIDTransport());
	...
	static bool HandleNpcSetSpawnDistCommand(ChatHandler* handler, const char* args) 
	{
		...
		WorldDatabase.PExecute("UPDATE creature SET spawndist=%f, MovementType=%i WHERE guid=%u",
	...
	static bool HandleNpcUnFollowCommand(ChatHandler* handler, const char* /*args*/) 
	{
		...
		if (/*creature->GetMotionMaster()->empty() ||*/ creature->GetMotionMaster()->GetCurrentMovementGeneratorType() != TARGETED_MOTION_TYPE) 
		{
	...
	static bool HandleNpcAddFormationCommand(ChatHandler* handler, const char* args) 
	{
		...
		{
			handler->PSendSysMessage("Selected creature is already member of group %u",
