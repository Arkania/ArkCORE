		...
		void UpdateAI(const uint32 diff) {
			...
			if (me->getVictim() && me->isAlive()) {
				...
				if (!Spawned && SpawnStartSpiders_Timer <= diff) {
					...
					Spider = me->SummonCreature(15041, pTarget->GetPositionX(),
				...
				if (SpawnSpider_Timer <= diff) {
					...
					Creature *Spider = me->SummonCreature(15041,
				...
				if (PhaseTwo) {
					...
					if (TransformBack_Timer <= diff) {
						...
						const CreatureInfo *cinfo = me->GetCreatureInfo();
						me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
								(cinfo->mindmg + ((cinfo->mindmg / 100) * 1)));
						me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE,
								(cinfo->maxdmg + ((cinfo->maxdmg / 100) * 1)));
						me->UpdateDamagePhysical(BASE_ATTACK);
...
public:
	...
	struct mob_spawn_of_marliAI: public ScriptedAI {
		mob_spawn_of_marliAI(Creature *c) :
				ScriptedAI(c) {
		}
