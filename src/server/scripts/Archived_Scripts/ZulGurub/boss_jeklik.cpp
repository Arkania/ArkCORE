			...
			if (me->getVictim() && me->isAlive()) {
				if (HealthAbovePct(50)) {
					...
					if (SpawnBats_Timer <= diff) {
						...
						Creature* Bat = NULL;
						Bat = me->SummonCreature(11368, -12291.6220f,
								-1380.2640f, 144.8304f, 5.483f,
								TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
						if (pTarget && Bat)
							Bat->AI()->AttackStart(pTarget);

						Bat = me->SummonCreature(11368, -12289.6220f,
								-1380.2640f, 144.8304f, 5.483f,
								TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
						if (pTarget && Bat)
							Bat->AI()->AttackStart(pTarget);

						Bat = me->SummonCreature(11368, -12293.6220f,
								-1380.2640f, 144.8304f, 5.483f,
								TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
						if (pTarget && Bat)
							Bat->AI()->AttackStart(pTarget);
					...
					if (PhaseTwo) {
						...
						if (SpawnFlyingBats_Timer <= diff) {
							...
							Creature* FlyingBat = me->SummonCreature(14965,
									pTarget->GetPositionX(),
									pTarget->GetPositionY(),
									pTarget->GetPositionZ() + 15, 0,
