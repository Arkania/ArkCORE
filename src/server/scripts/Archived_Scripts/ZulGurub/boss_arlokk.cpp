...
enum eYells {
	SAY_AGGRO = -1309011, SAY_FEAST_PANTHER = -1309012, SAY_DEATH = -1309013,
};
...
public:
	...
	struct boss_arlokkAI: public ScriptedAI {
		boss_arlokkAI(Creature* pCreature) :
		...
		void DoSummonPhanters() {
			...
			me->SummonCreature(NPC_ZULIAN_PROWLER, -11532.7998f, -1649.6734f,
					41.4800f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
					15000);
			me->SummonCreature(NPC_ZULIAN_PROWLER, -11532.9970f, -1606.4840f,
					41.2979f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
					15000);
		}
			...
			if (!m_bIsPhaseTwo) {
				...
				if (m_uiMark_Timer <= uiDiff) {
					...
					if (pMarkedTarget) {
						...
						sLog.outError(
								"TSCR: boss_arlokk could not accuire pMarkedTarget.");
			...
			if (m_bIsVanished) {
				if (m_uiVisible_Timer <= uiDiff) {
					...
					me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
							(cinfo->mindmg + ((cinfo->mindmg / 100) * 35)));
					me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE,
							(cinfo->maxdmg + ((cinfo->maxdmg / 100) * 35)));
	...
	bool OnGossipHello(Player* /*pPlayer*/, GameObject* pGo) {
		if (InstanceScript* m_pInstance = pGo->GetInstanceScript()) {
			if (m_pInstance->GetData(TYPE_ARLOKK) == DONE
					|| m_pInstance->GetData(TYPE_ARLOKK) == IN_PROGRESS)
				return true;
