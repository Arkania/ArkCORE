	...
	struct boss_jindoAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (HealingWard_Timer <= diff) {
				...
				me->SummonCreature(14987, me->GetPositionX() + 3,
						me->GetPositionY() - 2, me->GetPositionZ(), 0,
			...
			if (Delusions_Timer <= diff) {
				if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0)) {
					...
					Creature *Shade = me->SummonCreature(14986,
			...
			if (Teleport_Timer <= diff) {
				...
				if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER) {
					DoTeleportPlayer(pTarget, -11583.7783f, -1249.4278f,
							77.5471f, 4.745f);
					...
					Creature *Skeletons;
					Skeletons = me->SummonCreature(14826,
							pTarget->GetPositionX() + 2,
...
public:
	...
	struct mob_healing_wardAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 diff) {
			...
			if (Heal_Timer <= diff) {
				if (pInstance) {
					Unit *pJindo = Unit::GetUnit((*me),
							pInstance->GetData64(DATA_JINDO));
					if (pJindo)
...
public:
	...
	struct mob_shade_of_jindoAI: public ScriptedAI {
		mob_shade_of_jindoAI(Creature *c) :
				ScriptedAI(c) {
		}
