				...
				if (m_pInstance) {
					if (m_pInstance->GetData(TYPE_LORKHAN) == SPECIAL) {
						...
						if (Unit *pLorKhan = Unit::GetUnit((*me), m_pInstance->GetData64(DATA_LORKHAN))) {
							...
							pLorKhan->RemoveFlag(UNIT_FIELD_FLAGS,
					...
					if (m_pInstance->GetData(TYPE_ZATH) == SPECIAL) {
						//Resurrect Zath
						Unit *pZath = Unit::GetUnit((*me),
								m_pInstance->GetData64(DATA_ZATH));
						if (pZath) {
							pZath->SetUInt32Value(UNIT_FIELD_BYTES_1, 0);
							pZath->setFaction(14);
							pZath->RemoveFlag(UNIT_FIELD_FLAGS,
									UNIT_FLAG_NOT_SELECTABLE);
							pZath->SetFullHealth();
			...
			if (!PhaseTwo && WasDead) {
				if (Resurrect_Timer <= diff) {
					...
					me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE,
							(cinfo->mindmg + ((cinfo->mindmg / 100) * 40)));
					me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE,
							(cinfo->maxdmg + ((cinfo->maxdmg / 100) * 40)));
...
public:
	...
	struct mob_zealot_lorkhanAI: public ScriptedAI {
		mob_zealot_lorkhanAI(Creature *c) :
				ScriptedAI(c) {
			m_pInstance = c->GetInstanceScript();
		}
		...
		void UpdateAI(const uint32 diff) {
			...
			if (GreaterHeal_Timer <= diff) {
				if (m_pInstance) {
					Unit *pThekal = Unit::GetUnit((*me),
							m_pInstance->GetData64(DATA_THEKAL));
					Unit *pZath = Unit::GetUnit((*me),
							m_pInstance->GetData64(DATA_ZATH));
				...
				if (m_pInstance) {
					if (m_pInstance->GetData(TYPE_THEKAL) == SPECIAL) {
						...
						if (Unit *pThekal = Unit::GetUnit((*me), m_pInstance->GetData64(DATA_THEKAL))) {
							...
							pThekal->RemoveFlag(UNIT_FIELD_FLAGS,
									UNIT_FLAG_NOT_SELECTABLE);
