		...
		void UpdateAI(const uint32 diff) {
			...
			if (me->getVictim() && me->isAlive()) {
				if (!CombatStart) {
					...
					me->SummonCreature(14988, me->getVictim()->GetPositionX(),
							me->getVictim()->GetPositionY(),
							me->getVictim()->GetPositionZ(), 0,
							TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 35000);
					CombatStart = true;
				}
				...
				if (!someWatched) {
					...
					if (me->getVictim()
							&& me->getVictim()->HealthBelowPct(50)) {
