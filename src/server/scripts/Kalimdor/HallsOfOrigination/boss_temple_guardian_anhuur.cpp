...
enum ScriptTexts {
	SAY_AGGRO = 0, SAY_BEACON = 1, SAY_KILL_1 = 2, SAY_KILL_2 = 3, SAY_DEATH = 4,
};
...
enum BossPhases { //Don't use phase conditionals for the moment
...
public:
	boss_temple_guardian_anhuur() :
			CreatureScript("boss_temple_guardian_anhuur") {
	}
	...
	struct boss_temple_guardian_anhuurAI: public ScriptedAI {
		boss_temple_guardian_anhuurAI(Creature* pCreature) :
		...
		void Reset() {
			...
			if (pInstance
					&& (pInstance->GetData(DATA_TEMPLE_GUARDIAN_ANHUUR_EVENT)
							!= DONE && !check_in))
				pInstance->SetData(DATA_TEMPLE_GUARDIAN_ANHUUR_EVENT,
						NOT_STARTED);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_DIVINE_RECKONING:
					...
					events.ScheduleEvent(EVENT_DIVINE_RECKONING,
							urand(4000, 9000));
					break;
