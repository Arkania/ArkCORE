...
public:
	instance_halls_of_origination() :
			InstanceMapScript("instance_halls_of_origination", 644) {
	}
	...
	struct instance_halls_of_origination_InstanceMapScript: public InstanceScript {
		instance_halls_of_origination_InstanceMapScript(InstanceMap *map) :
				InstanceScript(map) {
		}
		...
		void OnGameObjectCreate(GameObject* go) {
			...
			case GO_ORIGINATION_ELEVATOR:
				...
				if (GetData(DATA_TEMPLE_GUARDIAN_ANHUUR) == DONE
						&& GetData(DATA_ANRAPHET) == DONE
						&& GetData(DATA_EARTHRAGER_PTAH) == DONE) {
		...
		std::string GetSaveData() {
			...
			saveStream << "H O" << uiEncounter[0] << " " << uiEncounter[1]
					<< " " << uiEncounter[2] << " " << uiEncounter[3] << " "
					<< uiEncounter[4] << " " << uiEncounter[5] << " "
					<< uiEncounter[6];
