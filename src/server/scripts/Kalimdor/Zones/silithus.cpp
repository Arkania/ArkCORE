...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(7785) == QUEST_STATUS_NONE
				&& (pPlayer->HasItemCount(18563, 1, false)
						|| pPlayer->HasItemCount(18564, 1, false)))
...
static QuestCinematic EventAnim[] = {
		...
		{ ANACHRONOS_EMOTE_2, 0, 3000 }, { ANACHRONOS_EMOTE_3, 0, 3000 }, {
				NULL, 0, 2000 }, { NULL, 0, 2000 }, { NULL, 0, 4000 }, {
				ANACHRONOS_SAY_10, 0, 3000 }, { NULL, 0, 2000 },
		{ NULL, 0, 3000 }, { NULL, 0, 15000 }, { NULL, 0, 5000 }, { NULL, 0,
				3500 }, { NULL, 0, 5000 }, { NULL, 0, 3500 }, { NULL, 0, 5000 },
		{ NULL, 0, NULL } };
...
static Location SpawnLocation[] = { { -8085, 1528, 2.61f, 3.141592f }, //Kaldorei Infantry
...
static WaveData WavesInfo[] = { { 30, 0, 15423, 0, 0, 24000, NULL }, //Kaldorei Soldier
...
public:
	...
	struct npc_anachronos_the_ancientAI: public ScriptedAI {
		...
		void HandleAnimation() {
			...
			Unit* Fandral = plr->FindNearestCreature(C_FANDRAL_STAGHELM, 100,
					me);
			Unit* Arygos = plr->FindNearestCreature(C_ARYGOS, 100, me);
			Unit* Caelestrasz = plr->FindNearestCreature(C_CAELESTRASZ, 100,
					me);
			...
			if (eventEnd == false) {
				...
				case 4:
					...
					break;
				case 5:
					Caelestrasz->SetUInt64Value(UNIT_FIELD_TARGET,
							Fandral->GetGUID());
	...
	struct npc_anachronos_quest_triggerAI: public ScriptedAI {
		...
		void SummonNextWave() {
			...
			for (uint8 i = 0; i < 67; ++i) {
				...
				Spawn = me->SummonCreature(WavesInfo[WaveCount].CreatureId, X,
						Y, Z, O, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, desptimer);
				...
				if (Spawn) {
					...
					if (WaveCount < 5) //1-4 Wave
							{
						CAST_AI(mob_qiraj_war_spawn::mob_qiraj_war_spawnAI, Spawn->AI())->MobGUID =
								me->GetGUID();
						CAST_AI(mob_qiraj_war_spawn::mob_qiraj_war_spawnAI, Spawn->AI())->PlayerGUID =
								PlayerGUID;
					}
				}
			}
		...
		void CheckEventFail() {
			...
			if (Group *EventGroup = pPlayer->GetGroup()) {
				...
				if (GroupMemberCount == FailedMemberCount
						|| !pPlayer->IsWithinDistInMap(me, EVENT_AREA_RADIUS))
