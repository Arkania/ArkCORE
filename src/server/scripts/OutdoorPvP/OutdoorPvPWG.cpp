...
i    < sAreaPOIStore.GetNumRows(); ++i)
    ...
    result = WorldDatabase.PQuery("SELECT guid FROM gameobject,gameobject_template"
            ...
            " AND gameobject.id=gameobject_template.entry", minX, minY, maxX, maxY);
    if (!result)
        return false;
...
OutdoorPvPWGCreType OutdoorPvPWG::GetCreatureType (uint32 entry) const
{
    ...
    case 31101:
    case 31051:          // Hoodoo Master & Sorceress
    case 31102:
...
bool OutdoorPvPWG::UpdateGameObjectInfo (GameObject *go) const
{
    ...
    {
        go->SetUInt32Value(GAMEOBJECT_DISPLAYID, getDefenderTeam() == TEAM_ALLIANCE ? itr->second : itr->first);
        return true;
    }
    return false;
}
...
{
    m_qgPosMap[std::pair<uint32, bool>(guid, true)] = posHorde, m_qgPosMap[std::pair<uint32, bool>(guid, false)] = posAlli, m_questgivers[guid] = NULL;
    if (getDefenderTeam() == TEAM_ALLIANCE)
...
OPvPCapturePointWG::OPvPCapturePointWG (OutdoorPvPWG *opvp, BuildingState *state) :
        OPvPCapturePoint(opvp), m_spiGuid(0), m_spiritguide(NULL), m_spiritguide_horde(NULL), m_spiritguide_alliance(NULL), m_engGuid(0), m_engineer(NULL), m_buildingState(state), m_wintergrasp(opvp)
{
}
