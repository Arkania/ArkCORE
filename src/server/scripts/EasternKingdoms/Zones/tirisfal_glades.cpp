...
enum eCalvin {
    SAY_COMPLETE = -1000431, SPELL_DRINK = 2639, // possibly not correct spell (but iconId is correct)
    QUEST_590 = 590,
    FACTION_HOSTILE = 168
};
...
public:
    ...
    bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
        ...
        if (pQuest->GetQuestId() == QUEST_590) {
            ...
            CAST_AI(npc_calvin_montague::npc_calvin_montagueAI, pCreature->AI())->AttackStart(
                    pPlayer);
        }
        return true;
    }
    ...
    struct npc_calvin_montagueAI: public ScriptedAI {
        npc_calvin_montagueAI(Creature* pCreature) :
...
enum eMausoleum {
    QUEST_ULAG = 1819, NPC_ULAG = 6390, GO_TRIGGER = 104593, GO_DOOR = 176594
};
