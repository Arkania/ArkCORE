...
static Dialogue OzDialogue[] = { { -1532103, 6000 }, { -1532104, 18000 }, {
        -1532105, 9000 }, { -1532106, 15000 } };

static Dialogue HoodDialogue[] = { { -1532107, 6000 }, { -1532108, 10000 }, {
        -1532109, 14000 }, { -1532110, 15000 } };

static Dialogue RAJDialogue[] = { { -1532111, 5000 }, { -1532112, 7000 }, {
        -1532113, 14000 }, { -1532114, 14000 } };

// Entries and spawn locations for creatures in Oz event
float Spawns[6][2] = { { 17535, -10896 }, // Dorothee
        { 17546, -10891 }, // Roar
        { 17547, -10884 }, // Tinhead
...
public:
    npc_barnes() :
            CreatureScript("npc_barnes") {
    }

    struct npc_barnesAI: public npc_escortAI {
        npc_barnesAI(Creature* c) :
                npc_escortAI(c) {
            RaidWiped = false;
            m_uiEventId = 0;
        ...
        void WaypointReached(uint32 i) {
            ...
            case 0:
                ...
                pInstance->DoUseDoorOrButton(
                        pInstance->GetData64(DATA_GO_STAGEDOORLEFT));
                break;
            case 4:
                TalkCount = 0;
                ...
                if (Creature* pSpotlight = me->SummonCreature(CREATURE_SPOTLIGHT,
                    ...
                    pSpotlight->SetFlag(UNIT_FIELD_FLAGS,
            ...
            case 9:
                PrepareEncounter();
                pInstance->DoUseDoorOrButton(
                        pInstance->GetData64(DATA_GO_CURTAINS));
                break;
            }
        }
        ...
        void PrepareEncounter() {
            sLog->outDebug(
                    LOG_FILTER_TSCR,
                    "TSCR: Barnes Opera Event - Introduction complete - preparing encounter %d",
                    m_uiEventId);
            uint8 index = 0;
            uint8 count = 0;
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
        ...
        npc_barnesAI* pBarnesAI =
                CAST_AI(npc_barnes::npc_barnesAI, pCreature->AI());
        ...
        case GOSSIP_ACTION_INFO_DEF + 3:
            ...
            sLog->outString(
                    "TSCR: player (GUID " UI64FMTD ") manually set Opera event to EVENT_OZ",
                    pPlayer->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF + 4:
            ...
            sLog->outString(
                    "TSCR: player (GUID " UI64FMTD ") manually set Opera event to EVENT_HOOD",
                    pPlayer->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF + 5:
            ...
            sLog->outString(
                    "TSCR: player (GUID " UI64FMTD ") manually set Opera event to EVENT_RAJ",
                    pPlayer->GetGUID());
            break;
        }
...
public:
    ...
    struct npc_image_of_medivhAI: public ScriptedAI {
        npc_image_of_medivhAI(Creature* c) :
                ScriptedAI(c) {
            pInstance = c->GetInstanceScript();
        }
        ...
        void Reset() {
            ...
            if (pInstance && pInstance->GetData64(DATA_IMAGE_OF_MEDIVH) == 0) {
                ...
                (*me).GetMotionMaster()->MovePoint(1, MedivPos[0], MedivPos[1],
                        MedivPos[2]);
                Step = 0;
            } else {
        ...
        void StartEvent() {
            Step = 1;
            EventStarted = true;
            Creature* Arcanagos = me->SummonCreature(MOB_ARCANAGOS,
                    ArcanagosPos[0], ArcanagosPos[1], ArcanagosPos[2], 0,
                    TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000);
            if (!Arcanagos)
                return;
            ArcanagosGUID = Arcanagos->GetGUID();
            Arcanagos->AddUnitMovementFlag(MOVEMENTFLAG_LEVITATING);
            (*Arcanagos).GetMotionMaster()->MovePoint(0, ArcanagosPos[0],
                    ArcanagosPos[1], ArcanagosPos[2]);
        ...
        uint32 NextStep(uint32 Step) {
            ...
            case 2:
                if (arca)
                    CAST_CRE(arca)->MonsterYell(SAY_DIALOG_ARCANAGOS_2,
                            LANG_UNIVERSAL, NULL);
                return 20000;
            case 3:
            ...
            case 4:
                if (arca)
                    CAST_CRE(arca)->MonsterYell(SAY_DIALOG_ARCANAGOS_4,
                            LANG_UNIVERSAL, NULL);
                return 20000;
            case 5:
            ...
            case 6:
                if (arca)
                    CAST_CRE(arca)->MonsterYell(SAY_DIALOG_ARCANAGOS_6,
                            LANG_UNIVERSAL, NULL);
                return 10000;
            case 7:
            ...
            case 11:
                if (arca)
                    CAST_CRE(arca)->MonsterYell(SAY_DIALOG_ARCANAGOS_8,
                            LANG_UNIVERSAL, NULL);
                return 5000;
            case 12:
                arca->GetMotionMaster()->MovePoint(0, -11010.82f, -1761.18f,
                        156.47f);
                arca->setActive(true);
            ...
            case 14:
                ...
                return 50000;
            case 15:
                arca->DealDamage(arca, arca->GetHealth(), NULL, DIRECT_DAMAGE,
                        SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                return 5000;
            default:
                return 9999999;
