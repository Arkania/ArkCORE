		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_ARCTIC_BREATH:
					...
					events.ScheduleEvent(EVENT_ARCTIC_BREATH,
							urand(26000, 33000));
					break;
				default:
					break;
	...
	bool OnGossipHello(Player* player, Creature* creature) {
		if (!creature->FindCurrentSpellBySpellId(SPELL_HARVEST_BLIGHT_SPECIMEN)
			...
			if (player->HasAura(SPELL_ORANGE_BLIGHT_RESIDUE)
				...
				creature->CastSpell(creature, SPELL_HARVEST_BLIGHT_SPECIMEN,
						false);
		return false;
	}
};
...
public:
	DeathPlagueTargetSelector(Unit* _caster) :
			caster(_caster) {
	}
	...
	bool operator()(Unit* unit) {
		...
		if (unit->HasAura(SPELL_RECENTLY_INFECTED)
	...
	class spell_frost_giant_death_plague_SpellScript: public SpellScript {
		...
		void HandleScript(SpellEffIndex effIndex) {
			...
			if (GetHitUnit() != GetCaster())
				GetCaster()->CastSpell(GetHitUnit(), SPELL_DEATH_PLAGUE_AURA,
						true);
			else if (failed)
				GetCaster()->CastSpell(GetCaster(), SPELL_DEATH_PLAGUE_KILL,
						true);
		}

		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_frost_giant_death_plague_SpellScript::CountTargets, EFFECT_0, TARGET_UNIT_AREA_ALLY_SRC);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_frost_giant_death_plague_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_AREA_ALLY_SRC);
			OnEffect +=
					SpellEffectFn(spell_frost_giant_death_plague_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
		}
...
public:
	...
	class spell_icc_harvest_blight_specimen_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_icc_harvest_blight_specimen_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
			OnEffect +=
					SpellEffectFn(spell_icc_harvest_blight_specimen_SpellScript::HandleQuestComplete, EFFECT_1, SPELL_EFFECT_QUEST_COMPLETE);
		}
	};
...
public:
	...
	bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/) {
		...
		if (instance->GetData(DATA_COLDFLAME_JETS) == NOT_STARTED) {
			...
			player->GetMap()->PlayerRelocation(player, 4126.35f, 2769.23f,
					350.963f, 0.0f);
