...
enum Shadowmourne {
	QUEST_FROST_INFUSION = 24757, ITEM_SHADOW_S_EDGE = 49888,

	SPELL_FROST_INFUSION = 72292, SPELL_FROST_IMBUED_BLADE = 72290,
};

static Position const RimefangFlyPos = { 4413.309f, 2456.421f, 223.3795f,
		2.890186f };
static Position const RimefangLandPos = { 4413.309f, 2456.421f, 203.3848f,
		2.890186f };
static Position const SpinestalkerFlyPos = { 4418.895f, 2514.233f, 220.4864f,
		3.396045f };
static Position const SpinestalkerLandPos = { 4418.895f, 2514.233f, 203.3848f,
		3.396045f };
static Position const SindragosaSpawnPos = { 4818.700f, 2483.710f, 287.0650f,
		3.089233f };
static Position const SindragosaFlyPos = { 4475.190f, 2484.570f, 234.8510f,
		3.141593f };
static Position const SindragosaLandPos = { 4419.190f, 2484.570f, 203.3848f,
		3.141593f };
static Position const SindragosaAirPos = { 4475.990f, 2484.430f, 247.9340f,
		3.141593f };

class FrostwyrmLandEvent: public BasicEvent {
public:
	FrostwyrmLandEvent(Creature& _owner, Position const& _dest) :
			owner(_owner), dest(_dest) {
	}
...
public:
	...
	struct boss_sindragosaAI: public BossAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_TAIL_SMASH, 20000,
					EVENT_GROUP_LAND_PHASE);
			events.ScheduleEvent(EVENT_FROST_BREATH, urand(8000, 12000),
					EVENT_GROUP_LAND_PHASE);
			events.ScheduleEvent(EVENT_UNCHAINED_MAGIC, urand(9000, 14000),
					EVENT_GROUP_LAND_PHASE);
		...
		void DoAction(const int32 action) {
			if (action == ACTION_START_FROSTWYRM) {
				...
				float moveTime = me->GetExactDist(&SindragosaFlyPos)
						/ (me->GetSpeed(MOVE_FLIGHT) * 0.001f);
		...
		void MovementInform(uint32 type, uint32 point) {
			...
			case POINT_AIR_PHASE:
				me->CastCustomSpell(SPELL_ICE_TOMB_TARGET,
						SPELLVALUE_MAX_TARGETS, RAID_MODE<int32>(2, 5, 3, 6),
						false);
		...
		void SpellHitTarget(Unit* target, SpellEntry const* spell) {
			if (SpellEntry const* buffet = sSpellMgr->GetSpellForDifficultyFromSpell(sSpellStore.LookupEntry(70127), me))
				if (buffet->Id == spell->Id)
					if (Aura const* mysticBuffet = target->GetAura(spell->Id))
						mysticBuffetStack =
								std::max < uint8
										> (mysticBuffetStack, mysticBuffet->GetStackAmount());
			...
			if (Player* player = target->ToPlayer()) {
				if (SpellEntry const* breath = sSpellMgr->GetSpellForDifficultyFromSpell(sSpellStore.LookupEntry(isThirdPhase ? SPELL_FROST_BREATH_P2 : SPELL_FROST_BREATH_P1), me)) {
					if (player->GetQuestStatus(QUEST_FROST_INFUSION)
							!= QUEST_STATUS_REWARDED
							&& breath->Id == spell->Id) {
						if (Item* shadowsEdge = player->GetWeaponForAttack(BASE_ATTACK, true)) {
							if (!player->HasAura(SPELL_FROST_IMBUED_BLADE)
								...
								if (Aura* infusion = player->GetAura(SPELL_FROST_INFUSION)) {
									if (infusion->GetStackAmount() == 3) {
										player->CastSpell(player,
												SPELL_FROST_IMBUED_BLADE, true);
										player->RemoveAura(infusion);
									} else
										player->CastSpell(player,
												SPELL_FROST_INFUSION, true);
								} else
									player->CastSpell(player,
											SPELL_FROST_INFUSION, true);
							}
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_CLEAVE:
					...
					events.ScheduleEvent(EVENT_CLEAVE, urand(15000, 20000),
							EVENT_GROUP_LAND_PHASE);
					break;
				case EVENT_TAIL_SMASH:
					DoCast(me, SPELL_TAIL_SMASH);
					events.ScheduleEvent(EVENT_TAIL_SMASH, urand(27000, 32000),
							EVENT_GROUP_LAND_PHASE);
					break;
				case EVENT_FROST_BREATH:
					...
					events.ScheduleEvent(EVENT_FROST_BREATH,
							urand(20000, 25000), EVENT_GROUP_LAND_PHASE);
					break;
				case EVENT_UNCHAINED_MAGIC:
					Talk(SAY_UNCHAINED_MAGIC);
					DoCast(me, SPELL_UNCHAINED_MAGIC);
					events.ScheduleEvent(EVENT_UNCHAINED_MAGIC,
							urand(30000, 35000), EVENT_GROUP_LAND_PHASE);
					break;
				case EVENT_ICY_GRIP:
					DoCast(me, SPELL_ICY_GRIP);
					events.ScheduleEvent(EVENT_ICY_GRIP, urand(70000, 75000),
							EVENT_GROUP_LAND_PHASE);
				...
				case EVENT_BLISTERING_COLD:
					...
					events.ScheduleEvent(EVENT_BLISTERING_COLD_YELL, 5000,
							EVENT_GROUP_LAND_PHASE);
					break;
				...
				case EVENT_AIR_PHASE:
					...
					events.RescheduleEvent(EVENT_UNCHAINED_MAGIC,
							urand(55000, 60000), EVENT_GROUP_LAND_PHASE);
...
public:
	...
	struct npc_spinestalkerAI: public ScriptedAI {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_CLEAVE_SPINESTALKER,
					urand(10000, 15000));
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_BELLOWING_ROAR:
					...
					events.ScheduleEvent(EVENT_BELLOWING_ROAR,
							urand(25000, 30000));
					break;
...
public:
	...
	struct npc_rimefangAI: public ScriptedAI {
		...
		void Reset() {
			events.Reset();
			events.ScheduleEvent(EVENT_FROST_BREATH_RIMEFANG,
					urand(12000, 15000));
		...
		void UpdateAI(const uint32 diff) {
			...
			while (uint32 eventId = events.ExecuteEvent()) {
				...
				case EVENT_ICY_BLAST: {
					...
					float moveTime = me->GetExactDist(&RimefangFlyPos)
							/ (me->GetSpeed(MOVE_FLIGHT) * 0.001f);
					events.ScheduleEvent(EVENT_ICY_BLAST,
							uint64(moveTime) + urand(60000, 70000));
					events.ScheduleEvent(EVENT_ICY_BLAST_CAST,
							uint64(moveTime) + 250);
					break;
				}
...
public:
	...
	struct npc_sindragosa_trashAI: public ScriptedAI {
		npc_sindragosa_trashAI(Creature* creature) :
				ScriptedAI(creature) {
			frostwyrmId =
					(creature->GetHomePosition().GetPositionY() < 2484.35f) ?
							DATA_RIMEFANG : DATA_SPINESTALKER;
		...
		void Reset() {
			...
			if (me->GetEntry() == NPC_FROSTWARDEN_HANDLER) {
				...
				events.ScheduleEvent(EVENT_CONCUSSIVE_SHOCK,
						urand(8000, 10000));
			}
	...
	class spell_sindragosa_s_fury_SpellScript: public SpellScript {
		...
		void HandleDummy(SpellEffIndex effIndex) {
			...
			float resistance = float(
					GetHitUnit()->GetResistance(
							GetFirstSchoolInMask(
									SpellSchoolMask(
											GetSpellInfo()->SchoolMask))));
			uint32 minResistFactor = uint32(
					(resistance / (resistance + 510.0f)) * 10.0f) * 2;
			uint32 randomResist = urand(0, (9 - minResistFactor) * 100) / 100
					+ minResistFactor;

			uint32 damage = (uint32(GetEffectValue() / targetCount)
					* randomResist) / 10;

			SpellNonMeleeDamage damageInfo(GetCaster(), GetHitUnit(),
					GetSpellInfo()->Id, GetSpellInfo()->SchoolMask);
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_sindragosa_s_fury_SpellScript::HandleDummy, EFFECT_1, SPELL_EFFECT_DUMMY);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_sindragosa_s_fury_SpellScript::CountTargets, EFFECT_1, TARGET_UNIT_AREA_ENTRY_DST);
		}
...
public:
	...
	class spell_sindragosa_unchained_magic_SpellScript: public SpellScript {
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_sindragosa_unchained_magic_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENEMY_SRC);
		}
	};
		...
		void OnRemove(AuraEffect const* aurEff,
			...
			if (GetTargetApplication()->GetRemoveMode()
				...
				GetTarget()->CastCustomSpell(SPELL_BACKLASH,
						...
						GetTarget(), true, NULL, aurEff, GetCasterGUID());
		}

		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_sindragosa_instability_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
		}
	};
...
public:
	...
	class spell_sindragosa_frost_beacon_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_sindragosa_frost_beacon_AuraScript::PeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	...
	class spell_sindragosa_ice_tomb_SpellScript: public SpellScript {
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_sindragosa_ice_tomb_SpellScript::SummonTomb);
		}
	};
	...
	class spell_sindragosa_ice_tomb_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_sindragosa_ice_tomb_AuraScript::PeriodicTick, EFFECT_2, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	};
...
public:
	FrostBombTargetSelector(Unit* _caster,
			std::list<Creature*> const& _collisionList) :
			caster(_caster), collisionList(_collisionList) {
	}
	...
	bool operator()(Unit* unit) {
		...
		for (std::list<Creature*>::const_iterator itr = collisionList.begin();
				itr != collisionList.end(); ++itr)
...
public:
	spell_sindragosa_collision_filter() :
			SpellScriptLoader("spell_sindragosa_collision_filter") {
	}
	...
	class spell_sindragosa_collision_filter_SpellScript: public SpellScript {
		...
		void FilterTargets(std::list<Unit*>& unitList) {
			...
			GetCreatureListWithEntryInGrid(tombs, GetCaster(), NPC_ICE_TOMB,
					200.0f);
		...
		void Register() {
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_sindragosa_collision_filter_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENEMY_SRC);
		}
	};
...
public:
	...
	class spell_sindragosa_icy_grip_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_sindragosa_icy_grip_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
...
public:
	...
	class spell_rimefang_icy_blast_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_rimefang_icy_blast_SpellScript::HandleTriggerMissile, EFFECT_1, SPELL_EFFECT_TRIGGER_MISSILE);
		}
	};
...
public:
	explicit OrderWhelpTargetSelector(Creature* _owner) :
			owner(_owner) {
	}
	...
	bool operator()(Creature* creature) {
		if (!creature->AI()->GetData(DATA_WHELP_MARKER)
				&& creature->AI()->GetData(DATA_FROSTWYRM_OWNER)
						== owner->AI()->GetData(DATA_FROSTWYRM_OWNER))
			return false;
		return true;
	}
...
public:
	...
	class spell_frostwarden_handler_order_whelp_SpellScript: public SpellScript {
		...
		void HandleForcedCast(SpellEffIndex effIndex) {
			...
			GetCreatureListWithEntryInGrid(unitList, GetCaster(),
					NPC_FROSTWING_WHELP, 150.0f);
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_frostwarden_handler_order_whelp_SpellScript::HandleForcedCast, EFFECT_0, SPELL_EFFECT_FORCE_CAST);
			OnUnitTargetSelect +=
					SpellUnitTargetFn(spell_frostwarden_handler_order_whelp_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENEMY_DST);
		}
	};
...
public:
	...
	class spell_frostwarden_handler_focus_fire_SpellScript: public SpellScript {
		...
		void Register() {
			OnEffect +=
					SpellEffectFn(spell_frostwarden_handler_focus_fire_SpellScript::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
		}
	};
	...
	class spell_frostwarden_handler_focus_fire_AuraScript: public AuraScript {
		...
		void Register() {
			OnEffectPeriodic +=
					AuraEffectPeriodicFn(spell_frostwarden_handler_focus_fire_AuraScript::PeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
		}
	};
...
public:
	spell_trigger_spell_from_caster(char const* scriptName, uint32 _triggerId) :
			SpellScriptLoader(scriptName), triggerId(_triggerId) {
	}
	...
	public:
		spell_trigger_spell_from_caster_SpellScript(uint32 _triggerId) :
				SpellScript(), triggerId(_triggerId) {
		}
		...
		void Register() {
			AfterHit +=
					SpellHitFn(spell_trigger_spell_from_caster_SpellScript::HandleTrigger);
		}
