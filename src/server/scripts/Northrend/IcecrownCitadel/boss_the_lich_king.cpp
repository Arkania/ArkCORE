 ...
 *
 * Copyright (C) 2011 TrinityEMU <http://www.arkania.net/>
 *
    ...
    public:
        ...
        struct npc_defileAI : public Scripted_NoMovementAI
        {
            ...
            void Reset()
            {
                if (!alreadyReset)
                {
                    if (SpellEntry const* defileAuraSpellEntry = sSpellMgr->GetSpellForDifficultyFromSpell(sSpellMgr->GetSpellProto(SPELL_DEFILE), me))
    ...
    public:
        ...
        class spell_lich_king_necrotic_plague_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const * aurEff, AuraEffectHandleModes mode)
            {
                ...
                uint32 spellId = aurEff->GetSpellProto()->Effects[EFFECT_0].Effect;
    ...
    public:
        ...
        class spell_lich_king_defile_AuraScript : public AuraScript
        {
            ...
            void OnPeriodic(AuraEffect const* aurEff)
            {
                ...
                if (SpellEntry const* defileDamage = sSpellMgr->GetSpellForDifficultyFromSpell(sSpellMgr->GetSpellProto(SPELL_DEFILE_DAMAGE), caster))
                {
                    triggeredSpellId = defileDamage->Id;
                    triggeredSpellBaseDamage = (int32)(defileDamage->Effects[EFFECT_0].CalcValue() * (1.0f + (map->IsHeroic() ? 0.1f : 0.05f) * m_hitCount));
                }
                ...
                if (SpellEntry const* defileIncrease = sSpellMgr->GetSpellForDifficultyFromSpell(sSpellMgr->GetSpellProto(SPELL_DEFILE_INCREASE), caster))
                {
        ...
        class spell_lich_king_infection_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                if (!GetTarget()->isAlive() || GetTarget()->GetHealthPct() >= 90)
                {
                    ...
                    //    apmap.erase(it);
                    PreventDefaultAction();
                    GetTarget()->RemoveAurasDueToSpell(aurEff->GetSpellProto()->Effects[EFFECT_0].Effect);
                }
            }
        ...
        class spell_lich_king_valkyr_summon_AuraScript : public AuraScript
        {
            ...
            void HandleTriggerSpell(AuraEffect const* aurEff)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    uint32 triggerSpellId = aurEff->GetSpellProto()->Effects[aurEff->GetEffIndex()].TriggerSpell;
                    randomPos.m_positionZ = caster->GetPositionZ() + 6.0f;
                    caster->CastSpell(randomPos.GetPositionX(), randomPos.GetPositionY(), randomPos.GetPositionZ(), triggerSpellId, true, NULL);
                }
            }
    ...
    public:
        ...
        class spell_lich_king_vile_spirit_summon_AuraScript : public AuraScript
        {
            ...
            void OnPeriodic(AuraEffect const* aurEff)
            {
                ...
                uint32 triggeredSpell = aurEff->GetSpellProto()->Effects[aurEff->GetEffIndex()].TriggerSpell;
