...
uint32 entry_search[3] = { 186609, 186610, 186611 };
	...
	struct npc_dragonflayer_forge_masterAI: public ScriptedAI {
		...
		void CheckForge() {
			if (pInstance) {
				switch (fm_Type) {
				case 1:
					pInstance->SetData(EVENT_FORGE_1,
							me->isAlive() ? NOT_STARTED : DONE);
					break;
				case 2:
					pInstance->SetData(EVENT_FORGE_2,
							me->isAlive() ? NOT_STARTED : DONE);
					break;
				case 3:
					pInstance->SetData(EVENT_FORGE_3,
							me->isAlive() ? NOT_STARTED : DONE);
					break;
				}
			}
