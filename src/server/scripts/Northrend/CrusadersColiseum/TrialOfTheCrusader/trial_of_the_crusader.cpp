...
static _Messages _GossipMessage[] = { { MSG_BEASTS, GOSSIP_ACTION_INFO_DEF + 1,
		false, TYPE_BEASTS }, { MSG_JARAXXUS, GOSSIP_ACTION_INFO_DEF + 2, false,
		TYPE_JARAXXUS }, { MSG_CRUSADERS, GOSSIP_ACTION_INFO_DEF + 3, false,
		TYPE_CRUSADERS }, { MSG_VALKIRIES, GOSSIP_ACTION_INFO_DEF + 4, false,
		TYPE_VALKIRIES }, { MSG_LICH_KING, GOSSIP_ACTION_INFO_DEF + 5, false,
		TYPE_ANUBARAK }, { MSG_ANUBARAK, GOSSIP_ACTION_INFO_DEF + 6, true,
		TYPE_ANUBARAK } };
...
public:
	...
	struct npc_announcer_toc10AI: public ScriptedAI {
		npc_announcer_toc10AI(Creature* pCreature) :
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		for (i = 0; i < NUM_MESSAGES; i++) {
			if ((!_GossipMessage[i].state
					&& m_pInstance->GetData(_GossipMessage[i].encounter) != DONE)
					|| (_GossipMessage[i].state
							&& m_pInstance->GetData(_GossipMessage[i].encounter)
									== DONE)) {
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
		...
		case GOSSIP_ACTION_INFO_DEF + 2:
			if (Creature* pJaraxxus = Unit::GetCreature(*pCreature, m_pInstance->GetData64(NPC_JARAXXUS))) {
				...
				pJaraxxus->RemoveFlag(UNIT_FIELD_FLAGS,
		...
		case GOSSIP_ACTION_INFO_DEF + 3:
			if (m_pInstance->GetData(TYPE_CRUSADERS) != DONE) {
				if (pPlayer->GetTeam() == ALLIANCE)
					...
					m_pInstance->SetData(TYPE_EVENT, 3001);
		...
		case GOSSIP_ACTION_INFO_DEF + 5: {
			...
			Creature* pTemp = Unit::GetCreature((*pCreature),
					m_pInstance->GetData64(NPC_ANUBARAK));
			if (!pTemp || !pTemp->isAlive())
				pTemp = pCreature->SummonCreature(NPC_ANUBARAK,
						AnubarakLoc[0].GetPositionX(),
						AnubarakLoc[0].GetPositionY(),
						AnubarakLoc[0].GetPositionZ(), 3,
...
public:
	...
	boss_lich_king_toc() :
			CreatureScript("boss_lich_king_toc") {
	}

	struct boss_lich_king_tocAI: public ScriptedAI {
		boss_lich_king_tocAI(Creature *pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) pCreature->GetInstanceScript();
		}
			...
			if (m_uiUpdateTimer <= uiDiff) {
				...
				case 5080:
					...
					Creature* pTemp = Unit::GetCreature((*me),
							m_pInstance->GetData64(NPC_ANUBARAK));
					if (!pTemp || !pTemp->isAlive())
						pTemp = me->SummonCreature(NPC_ANUBARAK,
								AnubarakLoc[0].GetPositionX(),
								AnubarakLoc[0].GetPositionY(),
								AnubarakLoc[0].GetPositionZ(), 3,
								TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME);
...
public:
	...
	npc_fizzlebang_toc() :
			CreatureScript("npc_fizzlebang_toc") {
	}

	struct npc_fizzlebang_tocAI: public ScriptedAI {
		npc_fizzlebang_tocAI(Creature* pCreature) :
		...
		void Reset() {
			me->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
			m_uiPortalGUID = 0;
			me->GetMotionMaster()->MovePoint(1, ToCCommonLoc[10].GetPositionX(),
					ToCCommonLoc[10].GetPositionY() - 60,
					ToCCommonLoc[10].GetPositionZ());
		}
		...
		void MovementInform(uint32 uiType, uint32 uiId) {
			...
			case 1:
				...
				if (m_pInstance) {
					m_pInstance->DoUseDoorOrButton(
							m_pInstance->GetData64(GO_MAIN_GATE_DOOR));
			...
			if (m_uiUpdateTimer <= uiDiff) {
				...
				case 1130:
					...
					if (Unit* pTrigger = me->SummonCreature(NPC_TRIGGER, ToCCommonLoc[1].GetPositionX(), ToCCommonLoc[1].GetPositionY(), ToCCommonLoc[1].GetPositionZ(), 4.69494f, TEMPSUMMON_MANUAL_DESPAWN)) {
						...
						pTrigger->CastSpell(pTrigger, SPELL_WILFRED_PORTAL,
								false);
					}
				...
				case 1134:
					...
					if (Creature* pPortal = me->SummonCreature(NPC_WILFRED_PORTAL, ToCCommonLoc[1].GetPositionX(), ToCCommonLoc[1].GetPositionY(), ToCCommonLoc[1].GetPositionZ(), 4.71239f, TEMPSUMMON_MANUAL_DESPAWN)) {
						...
						pPortal->CastSpell(pPortal, SPELL_WILFRED_PORTAL,
								false);
				...
				case 1140:
					...
					if (Creature* pTemp = me->SummonCreature(NPC_JARAXXUS, ToCCommonLoc[1].GetPositionX(), ToCCommonLoc[1].GetPositionY(), ToCCommonLoc[1].GetPositionZ(), 5.0f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME)) {
						...
						pTemp->GetMotionMaster()->MovePoint(0,
								ToCCommonLoc[1].GetPositionX(),
								ToCCommonLoc[1].GetPositionY() - 10,
								ToCCommonLoc[1].GetPositionZ());
					}
...
public:
	...
	npc_tirion_toc() :
			CreatureScript("npc_tirion_toc") {
	}

	struct npc_tirion_tocAI: public ScriptedAI {
		npc_tirion_tocAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) me->GetInstanceScript();
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiUpdateTimer <= uiDiff) {
				...
				case 150:
					...
					if (m_pInstance->GetData(TYPE_BEASTS) != DONE) {
						me->SummonCreature(NPC_GORMOK,
								ToCCommonLoc[10].GetPositionX(),
								ToCCommonLoc[10].GetPositionY(),
								ToCCommonLoc[10].GetPositionZ(), 5,
								TEMPSUMMON_CORPSE_TIMED_DESPAWN,
								30 * IN_MILLISECONDS);
						if (Creature* pTemp = Unit::GetCreature((*me), m_pInstance->GetData64(NPC_GORMOK))) {
							pTemp->GetMotionMaster()->MovePoint(0,
									ToCCommonLoc[5].GetPositionX(),
									ToCCommonLoc[5].GetPositionY(),
									ToCCommonLoc[5].GetPositionZ());
				...
				case 210:
					if (m_pInstance->GetData(TYPE_BEASTS) != DONE) {
						me->SummonCreature(NPC_DREADSCALE,
								ToCCommonLoc[3].GetPositionX(),
								ToCCommonLoc[3].GetPositionY(),
								ToCCommonLoc[3].GetPositionZ(), 5,
								TEMPSUMMON_CORPSE_TIMED_DESPAWN,
								30 * IN_MILLISECONDS);
						me->SummonCreature(NPC_ACIDMAW,
								ToCCommonLoc[4].GetPositionX(),
								ToCCommonLoc[4].GetPositionY(),
								ToCCommonLoc[4].GetPositionZ(), 5,
								TEMPSUMMON_CORPSE_TIMED_DESPAWN,
								30 * IN_MILLISECONDS);
						if (Creature* pTemp = Unit::GetCreature((*me), m_pInstance->GetData64(NPC_DREADSCALE))) {
							pTemp->GetMotionMaster()->MovePoint(0,
									ToCCommonLoc[8].GetPositionX(),
									ToCCommonLoc[8].GetPositionY(),
									ToCCommonLoc[8].GetPositionZ());
							pTemp->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
							pTemp->SetFlag(
									UNIT_FIELD_FLAGS,
									UNIT_FLAG_NON_ATTACKABLE
						...
						if (Creature* pTemp = Unit::GetCreature((*me), m_pInstance->GetData64(NPC_ACIDMAW))) {
							pTemp->GetMotionMaster()->MovePoint(0,
									ToCCommonLoc[9].GetPositionX(),
									ToCCommonLoc[9].GetPositionY(),
									ToCCommonLoc[9].GetPositionZ());
							pTemp->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
							pTemp->SetFlag(
									UNIT_FIELD_FLAGS,
									UNIT_FLAG_NON_ATTACKABLE
				...
				case 1010:
					...
					m_pInstance->DoUseDoorOrButton(
							m_pInstance->GetData64(GO_MAIN_GATE_DOOR));
					me->SummonCreature(NPC_FIZZLEBANG,
							ToCCommonLoc[10].GetPositionX(),
							ToCCommonLoc[10].GetPositionY(),
							ToCCommonLoc[10].GetPositionZ(), 2,
							TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME);
					m_pInstance->SetData(TYPE_EVENT, 0);
					break;
				case 1180:
				...
				case 3061:
					...
					//Summoning crusaders
				case 3091:
					me->SummonCreature(NPC_CHAMPIONS_CONTROLLER,
							ToCCommonLoc[1]);
				...
				case 4010:
					...
					break;
				case 4015:
					me->SummonCreature(NPC_LIGHTBANE,
							ToCCommonLoc[3].GetPositionX(),
							ToCCommonLoc[3].GetPositionY(),
							ToCCommonLoc[3].GetPositionZ(), 5,
							TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME);
					if (Creature* pTemp = Unit::GetCreature((*me), m_pInstance->GetData64(NPC_LIGHTBANE))) {
						pTemp->GetMotionMaster()->MovePoint(0,
								ToCCommonLoc[6].GetPositionX(),
								ToCCommonLoc[6].GetPositionY(),
								ToCCommonLoc[6].GetPositionZ());
						pTemp->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
						me->SetReactState(REACT_PASSIVE);
					}
					me->SummonCreature(NPC_DARKBANE,
							ToCCommonLoc[4].GetPositionX(),
							ToCCommonLoc[4].GetPositionY(),
							ToCCommonLoc[4].GetPositionZ(), 5,
							TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME);
					if (Creature* pTemp = Unit::GetCreature((*me), m_pInstance->GetData64(NPC_DARKBANE))) {
						pTemp->GetMotionMaster()->MovePoint(0,
								ToCCommonLoc[7].GetPositionX(),
								ToCCommonLoc[7].GetPositionY(),
								ToCCommonLoc[7].GetPositionZ());
						pTemp->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
						me->SetReactState(REACT_PASSIVE);
					}
					m_uiUpdateTimer = 5000;
				...
				case 5005:
					...
					me->SummonCreature(NPC_LICH_KING_1,
							ToCCommonLoc[2].GetPositionX(),
							ToCCommonLoc[2].GetPositionY(),
							ToCCommonLoc[2].GetPositionZ(), 5);
					break;
				case 5020:
					...
					break;
				case 6000:
					me->NearTeleportTo(AnubarakLoc[0].GetPositionX(),
							AnubarakLoc[0].GetPositionY(),
							AnubarakLoc[0].GetPositionZ(), 4.0f);
					m_uiUpdateTimer = 20000;
...
public:
	...
	npc_garrosh_toc() :
			CreatureScript("npc_garrosh_toc") {
	}

	struct npc_garrosh_tocAI: public ScriptedAI {
		npc_garrosh_tocAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) me->GetInstanceScript();
		}
...
public:
	...
	npc_varian_toc() :
			CreatureScript("npc_varian_toc") {
	}

	struct npc_varian_tocAI: public ScriptedAI {
		npc_varian_tocAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) me->GetInstanceScript();
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			if (!m_pInstance)
				return;
			if (m_pInstance->GetData(TYPE_EVENT_NPC) != NPC_VARIAN)
				return;
