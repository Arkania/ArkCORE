...
public:
	...
	struct boss_jaraxxusAI: public ScriptedAI {
		...
		void Reset() {
			...
			m_uiFelLightningTimer = urand(10 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiIncinerateFleshTimer = urand(20 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
			m_uiNetherPowerTimer = 40 * IN_MILLISECONDS;
			m_uiLegionFlameTimer = 30 * IN_MILLISECONDS;
			m_uiTouchOfJaraxxusTimer = urand(10 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiSummonInfernalEruptionTimer <= uiDiff) {
				...
				uint8 i = urand(2, 3);
				me->SummonCreature(NPC_INFERNAL_VOLCANO,
						JaraxxusLoc[i].GetPositionX(),
						JaraxxusLoc[i].GetPositionY(),
						JaraxxusLoc[i].GetPositionZ(),
						TEMPSUMMON_CORPSE_DESPAWN);
				m_uiSummonInfernalEruptionTimer = 2 * MINUTE * IN_MILLISECONDS;
			} else
				m_uiSummonInfernalEruptionTimer -= uiDiff;

			if (m_uiSummonNetherPortalTimer <= uiDiff) {
				DoScriptText(EMOTE_NETHER_PORTAL, me);
				DoScriptText(SAY_NETHER_PORTAL, me);
				uint8 i = urand(2, 3);
				me->SummonCreature(NPC_NETHER_PORTAL,
						JaraxxusLoc[i].GetPositionX(),
						JaraxxusLoc[i].GetPositionY(),
						JaraxxusLoc[i].GetPositionZ(),
						TEMPSUMMON_CORPSE_DESPAWN);
				...
				m_uiSummonNetherPortalTimer -= uiDiff;

			if (m_uiFelFireballTimer <= uiDiff) {
				DoCastVictim(SPELL_FEL_FIREBALL);
				m_uiFelFireballTimer = urand(10 * IN_MILLISECONDS,
						15 * IN_MILLISECONDS);
			} else
			...
			if (getDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC
					&& m_uiTouchOfJaraxxusTimer <= uiDiff) {
...
public:
	mob_legion_flame() :
			CreatureScript("mob_legion_flame") {
	}
...
public:
	mob_infernal_volcano() :
			CreatureScript("mob_infernal_volcano") {
	}
	...
	struct mob_infernal_volcanoAI: public Scripted_NoMovementAI {
		mob_infernal_volcanoAI(Creature* pCreature) :
				Scripted_NoMovementAI(pCreature), Summons(me) {
			m_pInstance = (InstanceScript*) pCreature->GetInstanceScript();
			Reset();
		}
...
public:
	mob_fel_infernal() :
			CreatureScript("mob_fel_infernal") {
	}
	...
	struct mob_fel_infernalAI: public ScriptedAI {
		mob_fel_infernalAI(Creature* pCreature) :
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_pInstance
					&& m_pInstance->GetData(TYPE_JARAXXUS) != IN_PROGRESS)
				me->ForcedDespawn();
...
public:
	mob_nether_portal() :
			CreatureScript("mob_nether_portal") {
	}
	...
	struct mob_nether_portalAI: public ScriptedAI {
		mob_nether_portalAI(Creature* pCreature) :
				ScriptedAI(pCreature), Summons(me) {
			m_pInstance = (InstanceScript*) pCreature->GetInstanceScript();
			Reset();
		}
...
public:
	mob_mistress_of_pain() :
			CreatureScript("mob_mistress_of_pain") {
	}
