...
enum Summons {
	NPC_SNOBOLD_VASSAL = 34800, NPC_SLIME_POOL = 35176,
};
	...
	struct boss_gormokAI: public ScriptedAI {
		...
		void JustSummoned(Creature* summon) {
			if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true)) {
				if (summon->GetEntry() == NPC_SNOBOLD_VASSAL) {
					summon->GetMotionMaster()->MoveJump(target->GetPositionX(),
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiImpaleTimer <= uiDiff) {
				...
				m_uiImpaleTimer = urand(8 * IN_MILLISECONDS,
						10 * IN_MILLISECONDS);
			} else
			...
			if (m_uiStaggeringStompTimer <= uiDiff) {
				...
				m_uiStaggeringStompTimer = urand(20 * IN_MILLISECONDS,
						25 * IN_MILLISECONDS);
			} else
			...
			if (m_uiSummonTimer <= uiDiff) {
				if (m_uiSummonCount > 0) {
					me->SummonCreature(NPC_SNOBOLD_VASSAL, me->GetPositionX(),
							me->GetPositionY(), me->GetPositionZ(), 0,
							TEMPSUMMON_CORPSE_DESPAWN);
					DoScriptText(SAY_SNOBOLLED, me);
				}
				m_uiSummonTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
...
public:
	mob_snobold_vassal() :
			CreatureScript("mob_snobold_vassal") {
	}
						...
						if (Unit* pGormok = Unit::GetCreature(*me, m_pInstance->GetData64(NPC_GORMOK))) {
							if (pGormok->isAlive()) {
								...
								me->GetMotionMaster()->MoveJump(
										pGormok->GetPositionX(),
										pGormok->GetPositionY(),
										pGormok->GetPositionZ(), 15.0f, 15.0f);
							} else {
...
struct boss_jormungarAI: public ScriptedAI {
	...
	void UpdateAI(const uint32 uiDiff) {
		...
		if (m_pInstance
				&& m_pInstance->GetData(TYPE_NORTHREND_BEASTS) == SNAKES_SPECIAL
				&& !m_bEnraged) {
		...
		case 0: // Mobile
			if (m_uiBiteTimer <= uiDiff) {
				...
				m_uiBiteTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
			...
			if (m_uiSpewTimer <= uiDiff) {
				...
				m_uiSpewTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
		...
		case 1: // Submerge
			...
			me->GetMotionMaster()->MovePoint(0,
					ToCCommonLoc[1].GetPositionX() + urand(0, 80) - 40,
					ToCCommonLoc[1].GetPositionY() + urand(0, 80) - 40,
		...
		case 4: // Stationary
			if (m_uiSprayTimer <= uiDiff) {
				...
				m_uiSprayTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
			...
			if (m_uiSweepTimer <= uiDiff) {
				...
				m_uiSweepTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
...
public:
	...
	struct mob_slime_poolAI: public ScriptedAI {
		mob_slime_poolAI(Creature *pCreature) :
				ScriptedAI(pCreature) {
		}
...
public:
	...
	struct boss_icehowlAI: public ScriptedAI {
		...
		void Reset() {
			m_uiFerociousButtTimer = urand(15 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
			m_uiArticBreathTimer = urand(25 * IN_MILLISECONDS,
					40 * IN_MILLISECONDS);
		...
		void MovementInform(uint32 uiType, uint32 uiId) {
			...
			switch (uiId) {
			case 0: // JUMP!? Fuck! THAT'S BEEZARR! Would someone PLEASE make MotionMaster->Move* work better?
				if (me->GetDistance2d(ToCCommonLoc[1].GetPositionX(),
						ToCCommonLoc[1].GetPositionY()) < 6.0f) {
					// Middle of the room
					m_uiStage = 1;
				} else {
		...
		void KilledUnit(Unit *pWho) {
			if (pWho->GetTypeId() == TYPEID_PLAYER) {
				if (m_pInstance)
					m_pInstance->SetData(DATA_TRIBUTE_TO_IMMORTALITY_ELEGIBLE,
							0);
			}
		}
		...
		void EnterCombat(Unit* /*pWho*/) {
			if (m_pInstance)
				m_pInstance->SetData(TYPE_NORTHREND_BEASTS,
						ICEHOWL_IN_PROGRESS);
			me->SetInCombatWithZone();
		}

		void SpellHitTarget(Unit* target, const SpellEntry* spell) {
			if (spell->Id == SPELL_TRAMPLE
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			case 0:
				...
				if (m_uiWhirlTimer <= uiDiff) {
					...
					m_uiWhirlTimer = urand(15 * IN_MILLISECONDS,
							30 * IN_MILLISECONDS);
				} else
				...
				if (m_uiMassiveCrashTimer <= uiDiff) {
					me->GetMotionMaster()->MoveJump(
							...
							ToCCommonLoc[1].GetPositionY(),
							ToCCommonLoc[1].GetPositionZ(), 10.0f, 20.0f); // 1: Middle of the room
			...
			case 3:
				if (m_uiTrampleTimer <= uiDiff) {
					if (Unit* pTarget = Unit::GetPlayer(*me, m_uiTrampleTargetGUID)) {
						...
						me->GetMotionMaster()->MoveJump(
								2 * me->GetPositionX() - m_fTrampleTargetX,
								2 * me->GetPositionY() - m_fTrampleTargetY,
								me->GetPositionZ(), 10.0f, 20.0f); // 2: Hop Backwards
						m_uiStage = 7; //Invalid (Do nothing more than move)
					} else
						m_uiStage = 6;
				} else
			...
			case 4:
				...
				me->GetMotionMaster()->MoveCharge(m_fTrampleTargetX,
						m_fTrampleTargetY, m_fTrampleTargetZ + 2, 42, 1);
				...
				if (m_uiTrampleTimer <= uiDiff) {
					...
					for (Map::PlayerList::const_iterator itr = lPlayers.begin();
						...
						if (Unit* pPlayer = itr->getSource())
							if (pPlayer->isAlive()
									&& pPlayer->IsWithinDistInMap(me, 6.0f)) {
								DoCastAOE(SPELL_TRAMPLE);
								m_uiTrampleTimer = IN_MILLISECONDS;
								break;
							}
...
public:
	spell_gen_fire_bomb() :
			SpellScriptLoader("spell_gen_fire_bomb") {
	}
	...
	class spell_gen_fire_bomb_SpellScript: public SpellScript {
		...
		void HandleAfterHit() {
			...
			if (WorldLocation * pos = GetTargetDest())
				caster->SummonCreature(NPC_FIRE_BOMB, pos->GetPositionX(),
						pos->GetPositionY(), pos->GetPositionZ(), 0,
						TEMPSUMMON_TIMED_DESPAWN, 60000);
		}

		void Register() {
			AfterHit +=
					SpellHitFn(spell_gen_fire_bomb_SpellScript::HandleAfterHit);
		}
	};
