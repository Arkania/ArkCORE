	...
	struct instance_trial_of_the_crusader_InstanceMapScript: public InstanceScript {
		...
		void OnPlayerEnter(Player *m_player) {
			if (instance->IsHeroic()) {
				...
				m_player->SendUpdateWorldState(UPDATE_STATE_UI_COUNT,
						GetData(TYPE_COUNTER));
			}
		}
		...
		void OnCreatureCreate(Creature* pCreature, bool /*add*/) {
			...
			case NPC_BARRENT:
				m_uiBarrentGUID = pCreature->GetGUID();
				break;
			case NPC_TIRION:
			...
			case NPC_GARROSH:
				...
				break;
			case NPC_VARIAN:
				m_uiVarianGUID = pCreature->GetGUID();
				break;
			...
			case NPC_ICEHOWL:
				...
				break;
			case NPC_JARAXXUS:
				m_uiJaraxxusGUID = pCreature->GetGUID();
				break;
			case NPC_CHAMPIONS_CONTROLLER:
				m_uiChampionsControllerGUID = pCreature->GetGUID();
				break;
			case NPC_DARKBANE:
				m_uiDarkbaneGUID = pCreature->GetGUID();
				break;
			case NPC_LIGHTBANE:
				m_uiLightbaneGUID = pCreature->GetGUID();
				break;
			case NPC_ANUBARAK:
		...
		void OnGameObjectCreate(GameObject* pGO, bool /*bAdd*/) {
			...
			case GO_EAST_PORTCULLIS:
				m_uiEastPortcullisGUID = pGO->GetGUID();
				break;
			case GO_WEB_DOOR:
			...
			case GO_TRIBUTE_CHEST_25H_99:
				m_uiTributeChestGUID = pGO->GetGUID();
				break;
			}
		}
		...
		void SetData(uint32 uiType, uint32 uiData) {
			...
			case TYPE_VALKIRIES:
				...
				case FAIL:
					...
					break;
				case SPECIAL:
					if (m_auiEncounter[TYPE_VALKIRIES] == SPECIAL)
						uiData = DONE;
					break;
				case DONE:
					if (instance->GetPlayers().getFirst()->getSource()->GetTeam()
							== ALLIANCE)
						m_uiEvent = 4020;
					else
						m_uiEvent = 4030;
					...
					if (tributeChest)
						if (Creature* pTirion = instance->GetCreature(m_uiTirionGUID))
							if (GameObject* pChest = pTirion->SummonGameObject(tributeChest, 805.62f, 134.87f, 142.16f, 3.27f, 0, 0, 0, 0, 90000000))
								pChest->SetRespawnTime(
										pChest->GetRespawnDelay());
					break;
				}
				break;
			case TYPE_COUNTER:
				m_uiTrialCounter = uiData;
				uiData = DONE;
				break;
			case TYPE_EVENT:
				m_uiEvent = uiData;
			...
			case TYPE_NORTHREND_BEASTS:
				...
				case SNAKES_IN_PROGRESS:
					...
					break;
				case SNAKES_SPECIAL:
					m_uiNotOneButTwoJormungarsTimer = 10 * IN_MILLISECONDS;
					break;
				case SNAKES_DONE:
			...
			if (uiType < MAX_ENCOUNTERS) {
				sLog->outBasic(
						"[ToCr] m_auiEncounter[uiType %u] %u = uiData %u;",
						uiType, m_auiEncounter[uiType], uiData);
				if (uiData == FAIL) {
					if (IsRaidWiped()) {
						--m_uiTrialCounter;
						m_bNeedSave = true;
				...
				if (uiData == DONE || m_bNeedSave == true) {
					if (Unit* pAnnouncer = instance->GetCreature(GetData64(NPC_BARRENT)))
						pAnnouncer->SetFlag(UNIT_NPC_FLAGS,
		...
		uint64 GetData64(uint32 uiData) {
			...
			case GO_MAIN_GATE_DOOR:
				return m_uiMainGateDoorGUID;
			case GO_EAST_PORTCULLIS:
				return m_uiEastPortcullisGUID;
			case GO_WEB_DOOR:
				return m_uiWebDoorGUID;
			}
			return 0;
		}
		...
		uint32 GetData(uint32 uiType) {
			switch (uiType) {
			case TYPE_BEASTS:
				return m_auiEncounter[TYPE_BEASTS];
			case TYPE_JARAXXUS:
				return m_auiEncounter[TYPE_JARAXXUS];
			case TYPE_CRUSADERS:
				return m_auiEncounter[TYPE_CRUSADERS];
			case TYPE_VALKIRIES:
				return m_auiEncounter[TYPE_VALKIRIES];
			case TYPE_LICH_KING:
				return m_auiEncounter[TYPE_LICH_KING];
			case TYPE_ANUBARAK:
				return m_auiEncounter[TYPE_ANUBARAK];
			case TYPE_COUNTER:
				return m_uiTrialCounter;
			case TYPE_EVENT:
				return m_uiEvent;
			case TYPE_NORTHREND_BEASTS:
				return m_uiNorthrendBeasts;
			case TYPE_EVENT_TIMER:
		...
		void Update(uint32 uiDiff) {
			if (GetData(TYPE_NORTHREND_BEASTS) == SNAKES_SPECIAL
				...
				if (m_uiNotOneButTwoJormungarsTimer <= uiDiff)
					...
					m_uiNotOneButTwoJormungarsTimer -= uiDiff;
			}

			if (GetData(TYPE_CRUSADERS) == IN_PROGRESS
					&& m_uiResilienceWillFixItTimer) {
				if (m_uiResilienceWillFixItTimer <= uiDiff)
					m_uiResilienceWillFixItTimer = 0;
				else
					m_uiResilienceWillFixItTimer -= uiDiff;
			}
		}
