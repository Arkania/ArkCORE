...
enum Summons {
	NPC_DARK_ESSENCE = 34567, NPC_LIGHT_ESSENCE = 34568,
...
struct boss_twin_baseAI: public ScriptedAI {
	boss_twin_baseAI(Creature* pCreature) :
	...
	void MovementInform(uint32 uiType, uint32 uiId) {
		...
		switch (uiId) {
		case 0:
			me->GetMotionMaster()->MovePoint(1, HomeLocation.GetPositionX(),
					HomeLocation.GetPositionY(), HomeLocation.GetPositionZ());
	...
	void DamageTaken(Unit* pDoneBy, uint32 &uiDamage) {
		...
		if (m_pInstance)
			m_pInstance->SetData(
					...
					me->GetHealth() >= uiDamage ?
							me->GetHealth() - uiDamage : 0);
	}
	...
	void SummonColorballs(uint8 quantity) {
		float x0 = ToCCommonLoc[1].GetPositionX(), y0 =
				ToCCommonLoc[1].GetPositionY(), r = 47.0f;
		float y = y0;
		for (uint8 i = 0; i < quantity; i++) {
			...
			if (urand(0, 1))
				...
				y = y0 - sqrt(pow(r, 2) - pow((x - x0), 2));
			me->SummonCreature(m_uiColorballNpcId, x, y, me->GetPositionZ(),
	...
	void EnterCombat(Unit* /*pWho*/) {
		...
		if (me->isAlive()) {
			me->SummonCreature(m_uiEssenceNpcId,
					EssenceLocation[0].GetPositionX(),
					EssenceLocation[0].GetPositionY(),
					EssenceLocation[0].GetPositionZ());
			me->SummonCreature(m_uiEssenceNpcId,
					EssenceLocation[1].GetPositionX(),
					EssenceLocation[1].GetPositionY(),
					EssenceLocation[1].GetPositionZ());
		}
...
public:
	...
	struct boss_fjolaAI: public boss_twin_baseAI {
		...
		void Reset() {
			...
			SetEquipmentSlots(false, EQUIP_MAIN_1, EQUIP_OFFHAND_1,
					EQUIP_RANGED_1);
			m_uiStage = 0;
			...
			if (m_pInstance) {
				m_pInstance->DoStopTimedAchievement(
						ACHIEVEMENT_TIMED_TYPE_EVENT, EVENT_START_TWINS_FIGHT);
			}
		}
...
public:
	...
	struct boss_eydisAI: public boss_twin_baseAI {
		...
		void Reset() {
			boss_twin_baseAI::Reset();
			SetEquipmentSlots(false, EQUIP_MAIN_2, EQUIP_OFFHAND_2,
					EQUIP_RANGED_2);
			m_uiStage = 1;
...
public:
	mob_essence_of_twin() :
			CreatureScript("mob_essence_of_twin") {
	}
...
struct mob_unleashed_ballAI: public ScriptedAI {
	mob_unleashed_ballAI(Creature *pCreature) :
...
public:
	mob_unleashed_dark() :
			CreatureScript("mob_unleashed_dark") {
	}
	...
	struct mob_unleashed_darkAI: public mob_unleashed_ballAI {
		mob_unleashed_darkAI(Creature *pCreature) :
				mob_unleashed_ballAI(pCreature) {
		}
...
public:
	mob_unleashed_light() :
			CreatureScript("mob_unleashed_light") {
	}
	...
	struct mob_unleashed_lightAI: public mob_unleashed_ballAI {
		mob_unleashed_lightAI(Creature *pCreature) :
				mob_unleashed_ballAI(pCreature) {
		}
