...
enum SummonActions {
	ACTION_SHADOW_STRIKE, ACTION_SCARAB_SUBMERGE,
};

const Position SphereSpawn[6] = { { 786.6439f, 108.2498f, 155.6701f, 0 }, {
		806.8429f, 150.5902f, 155.6701f, 0 }, { 759.1386f, 163.9654f, 155.6701f,
		0 }, { 744.3701f, 119.5211f, 155.6701f, 0 }, { 710.0211f, 120.8152f,
		155.6701f, 0 }, { 706.6383f, 161.5266f, 155.6701f, 0 }, };

class boss_anubarak_trial: public CreatureScript {
public:
	boss_anubarak_trial() :
			CreatureScript("boss_anubarak_trial") {
	}
	...
	struct boss_anubarak_trialAI: public ScriptedAI {
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			case 0:
				...
				if (m_uiSubmergeTimer <= uiDiff && !m_bReachedPhase3
						&& !me->HasAura(SPELL_BERSERK)) {
					m_uiStage = 1;
			...
			case 2:
				...
				if (m_uiSummonScarabTimer <= uiDiff) {
					...
					m_uiScarabSummoned++;
					m_uiSummonScarabTimer = 4 * IN_MILLISECONDS;
					if (m_uiScarabSummoned == 4)
						m_uiSummonScarabTimer = RAID_MODE(4, 20)
								* IN_MILLISECONDS;
			...
			if (!IsHeroic()) {
				if (m_uiSummonFrostSphereTimer <= uiDiff) {
					...
					m_uiSummonFrostSphereTimer = urand(20, 30)
							* IN_MILLISECONDS;
				} else
...
public:
	mob_swarm_scarab() :
			CreatureScript("mob_swarm_scarab") {
	}
	...
	struct mob_swarm_scarabAI: public ScriptedAI {
		mob_swarm_scarabAI(Creature* pCreature) :
		...
		void Reset() {
			me->SetCorpseDelay(0);
			m_uiDeterminationTimer = urand(5 * IN_MILLISECONDS,
					60 * IN_MILLISECONDS);
		...
		void JustDied(Unit* pKiller) {
			DoCast(pKiller,
					RAID_MODE(SPELL_TRAITOR_KING_10, SPELL_TRAITOR_KING_25));
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiDeterminationTimer <= uiDiff) {
				...
				m_uiDeterminationTimer = urand(10 * IN_MILLISECONDS,
						60 * IN_MILLISECONDS);
			} else
...
public:
	mob_nerubian_burrower() :
			CreatureScript("mob_nerubian_burrower") {
	}
	...
	struct mob_nerubian_burrowerAI: public ScriptedAI {
		mob_nerubian_burrowerAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) pCreature->GetInstanceScript();
		}
		...
		void Reset() {
			me->SetCorpseDelay(0);
			m_uiSpiderFrenzyTimer = urand(10 * IN_MILLISECONDS,
					20 * IN_MILLISECONDS);
...
public:
	mob_frost_sphere() :
			CreatureScript("mob_frost_sphere") {
	}
	...
	struct mob_frost_sphereAI: public ScriptedAI {
		mob_frost_sphereAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
public:
	mob_anubarak_spike() :
			CreatureScript("mob_anubarak_spike") {
	}
	...
	struct mob_anubarak_spikeAI: public ScriptedAI {
		mob_anubarak_spikeAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			m_pInstance = (InstanceScript*) pCreature->GetInstanceScript();
		}
