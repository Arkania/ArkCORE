...
enum eAIs {
	AI_MELEE = 0, AI_RANGED = 1, AI_HEALER = 2, AI_PET = 3,
};

enum eSpells {
	SPELL_ANTI_AOE = 68595, SPELL_PVP_TRINKET = 65547,
};

class boss_toc_champion_controller: public CreatureScript {
public:
	boss_toc_champion_controller() :
			CreatureScript("boss_toc_champion_controller") {
	}
	...
	struct boss_toc_champion_controllerAI: public ScriptedAI {
		boss_toc_champion_controllerAI(Creature* pCreature) :
		...
		std::vector<uint32> SelectChampions(Team playerTeam) {
			...
			vHealersEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_DRUID_RESTORATION :
							NPC_ALLIANCE_DRUID_RESTORATION);
			vHealersEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_PALADIN_HOLY : NPC_ALLIANCE_PALADIN_HOLY);
			vHealersEntries.push_back(
					playerTeam == ALLIANCE ?
			...
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_DEATH_KNIGHT : NPC_ALLIANCE_DEATH_KNIGHT);
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_HUNTER : NPC_ALLIANCE_HUNTER);
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_MAGE : NPC_ALLIANCE_MAGE);
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_ROGUE : NPC_ALLIANCE_ROGUE);
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_WARLOCK : NPC_ALLIANCE_WARLOCK);
			vOtherEntries.push_back(
					playerTeam == ALLIANCE ?
							NPC_HORDE_WARRIOR : NPC_ALLIANCE_WARRIOR);

			uint8 healersSubtracted = 2;
			if (m_pInstance->instance->GetSpawnMode()
					== RAID_DIFFICULTY_25MAN_NORMAL
					|| m_pInstance->instance->GetSpawnMode()
				...
				if (Creature* pTemp = me->SummonCreature(vChampionEntries[i], vChampionJumpOrigin[urand(0, vChampionJumpOrigin.size()-1)], TEMPSUMMON_MANUAL_DESPAWN)) {
					...
					if (playerTeam == ALLIANCE) {
						pTemp->SetHomePosition(
								vChampionJumpTarget[pos].GetPositionX(),
								vChampionJumpTarget[pos].GetPositionY(),
								vChampionJumpTarget[pos].GetPositionZ(), 0);
						pTemp->GetMotionMaster()->MoveJump(
								vChampionJumpTarget[pos].GetPositionX(),
								vChampionJumpTarget[pos].GetPositionY(),
								vChampionJumpTarget[pos].GetPositionZ(), 20.0f,
								20.0f);
						pTemp->SetOrientation(0);
					} else {
						pTemp->SetHomePosition(
								(ToCCommonLoc[1].GetPositionX() * 2)
										- vChampionJumpTarget[pos].GetPositionX(),
								vChampionJumpTarget[pos].GetPositionY(),
								vChampionJumpTarget[pos].GetPositionZ(), 3);
						pTemp->GetMotionMaster()->MoveJump(
								(ToCCommonLoc[1].GetPositionX() * 2)
										- vChampionJumpTarget[pos].GetPositionX(),
								vChampionJumpTarget[pos].GetPositionY(),
		...
		void SetData(uint32 uiType, uint32 uiData) {
			...
			case 0:
				...
				break;
			case 1:
				for (std::list<uint64>::iterator i = Summons.begin();
						i != Summons.end(); ++i) {
			...
			case 2:
				...
				case FAIL:
					m_uiChampionsFailed++;
					if (m_uiChampionsFailed + m_uiChampionsKilled
							>= Summons.size()) {
...
struct boss_faction_championsAI: public ScriptedAI {
	...
	void UpdateThreat() {
		std::list<HostileReference*> const& tList =
		...
		for (std::list<HostileReference*>::const_iterator itr = tList.begin();
			...
			if (pUnit && me->getThreatManager().getThreat(pUnit)) {
				if (pUnit->GetTypeId() == TYPEID_PLAYER) {
					float threat = CalculateThreat(me->GetDistance2d(pUnit),
							(float) pUnit->GetArmor(), pUnit->GetHealth());
			...
			if (m_pInstance) {
				if (TeamInInstance == ALLIANCE) {
					if (Creature* pTemp = Unit::GetCreature(*me, m_pInstance->GetData64(NPC_VARIAN)))
						DoScriptText(
								SAY_VARIAN_KILL_HORDE_PLAYER4 + urand(0, 3),
								pTemp); // + cause we are on negative
				} else if (Creature* pTemp = me->FindNearestCreature(NPC_GARROSH, 300.f))
					DoScriptText(
							SAY_GARROSH_KILL_ALLIANCE_PLAYER4 + urand(0, 3),
							pTemp); // + cause we are on negative
...
public:
	...
	struct mob_toc_druidAI: public boss_faction_championsAI {
		mob_toc_druidAI(Creature* pCreature) :
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiNatureGraspTimer <= uiDiff) {
				...
				m_uiNatureGraspTimer = urand(40 * IN_MILLISECONDS,
						80 * IN_MILLISECONDS);
			} else
			...
			if (m_uiTranquilityTimer <= uiDiff) {
				...
				m_uiTranquilityTimer = urand(40 * IN_MILLISECONDS,
						90 * IN_MILLISECONDS);
			} else
			...
			if (m_uiBarkskinTimer <= uiDiff) {
				...
				m_uiBarkskinTimer = urand(45 * IN_MILLISECONDS,
						90 * IN_MILLISECONDS);
			} else
			...
			if (m_uiCommonTimer <= uiDiff) {
				...
				m_uiCommonTimer = urand(15 * IN_MILLISECONDS,
						30 * IN_MILLISECONDS);
			} else
...
public:
	...
	struct mob_toc_shamanAI: public boss_faction_championsAI {
		mob_toc_shamanAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_HEALER) {
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiHeroismOrBloodlustTimer <= uiDiff) {
				...
				} else if (!me->HasAura(AURA_SATED))
					DoCastAOE(SPELL_BLOODLUST);
				m_uiHeroismOrBloodlustTimer = urand(30 * IN_MILLISECONDS,
						60 * IN_MILLISECONDS);
			} else
			...
			if (m_uiHexTimer <= uiDiff) {
				...
				m_uiHexTimer = urand(10 * IN_MILLISECONDS,
						40 * IN_MILLISECONDS);
			} else
				m_uiHexTimer -= uiDiff;
			...
			if (m_uiCommonTimer <= uiDiff) {
				...
				m_uiCommonTimer = urand(5 * IN_MILLISECONDS,
						15 * IN_MILLISECONDS);
			} else
				m_uiCommonTimer -= uiDiff;
...
public:
	mob_toc_paladin() :
			CreatureScript("mob_toc_paladin") {
	}
	...
	struct mob_toc_paladinAI: public boss_faction_championsAI {
		mob_toc_paladinAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_HEALER) {
		}
		...
		void Reset() {
			...
			m_uiHandOfProtectionTimer = urand(0 * IN_MILLISECONDS,
					360 * IN_MILLISECONDS);
			m_uiHolyShockTimer = urand(6 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiHandOfFreedomTimer = urand(25 * IN_MILLISECONDS,
					40 * IN_MILLISECONDS);
			m_uiHammerOfJusticeTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiBubbleTimer <= uiDiff) {
				...
				m_uiBubbleTimer = urand(0 * IN_MILLISECONDS,
						360 * IN_MILLISECONDS);
			} else
...
public:
	...
	struct mob_toc_priestAI: public boss_faction_championsAI {
		mob_toc_priestAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_HEALER) {
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiPsychicScreamTimer <= uiDiff) {
				...
				m_uiPsychicScreamTimer = urand(5 * IN_MILLISECONDS,
						25 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_shadow_priest() :
			CreatureScript("mob_toc_shadow_priest") {
	}
	...
	struct mob_toc_shadow_priestAI: public boss_faction_championsAI {
		mob_toc_shadow_priestAI(Creature *pCreature) :
		...
		void Reset() {
			...
			m_uiPsychicScreamTimer = urand(5 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
			m_uiDispersionTimer = urand(1 * IN_MILLISECONDS,
					180 * IN_MILLISECONDS);
			m_uiSilenceTimer = urand(8 * IN_MILLISECONDS, 15 * IN_MILLISECONDS);
			m_uiMindBlastTimer = urand(3 * IN_MILLISECONDS,
					8 * IN_MILLISECONDS);
			m_uiCommonTimer = urand(15 * IN_MILLISECONDS, 30 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiSilenceTimer <= uiDiff) {
				...
				m_uiSilenceTimer = urand(8 * IN_MILLISECONDS,
						15 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_warlock() :
			CreatureScript("mob_toc_warlock") {
	}
	...
	struct mob_toc_warlockAI: public boss_faction_championsAI {
		mob_toc_warlockAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_RANGED), Summons(me) {
		}
		...
		void Reset() {
			...
			m_uiHellfireTimer = urand(15 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
			m_uiUnstableAfflictionTimer = urand(2 * IN_MILLISECONDS,
					10 * IN_MILLISECONDS);
			m_uiCommonTimer = urand(15 * IN_MILLISECONDS, 30 * IN_MILLISECONDS);
			SetEquipmentSlots(false, 49992, EQUIP_NO_CHANGE, EQUIP_NO_CHANGE);

			m_uiSummonPetTimer = urand(15 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiFearTimer <= uiDiff) {
				...
				m_uiFearTimer = urand(4 * IN_MILLISECONDS,
						15 * IN_MILLISECONDS);
			} else
...
public:
	...
	struct mob_toc_mageAI: public boss_faction_championsAI {
		mob_toc_mageAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_RANGED) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiCounterspellTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiBlinkTimer = urand(7 * IN_MILLISECONDS, 25 * IN_MILLISECONDS);
			m_uiIceBlockTimer = urand(0 * IN_MILLISECONDS,
					360 * IN_MILLISECONDS);
			m_uiPolymorphTimer = urand(15 * IN_MILLISECONDS,
					40 * IN_MILLISECONDS);
			m_uiCommonTimer = urand(15 * IN_MILLISECONDS, 30 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiBlinkTimer <= uiDiff) {
				...
				m_uiBlinkTimer = urand(7 * IN_MILLISECONDS,
						25 * IN_MILLISECONDS);
			} else
...
enum eHunterSpells {
	SPELL_AIMED_SHOT = 65883, SPELL_DETERRENCE = 65871, //90s
...
public:
	...
	struct mob_toc_hunterAI: public boss_faction_championsAI {
		mob_toc_hunterAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_RANGED), Summons(me) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiDisengageTimer = urand(12 * IN_MILLISECONDS,
					20 * IN_MILLISECONDS);
			m_uiDeterrenceTimer = urand(20 * IN_MILLISECONDS,
					120 * IN_MILLISECONDS);
			m_uiWyvernStingTimer = urand(7 * IN_MILLISECONDS,
					60 * IN_MILLISECONDS);
			m_uiFrostTrapTimer = urand(12 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiWingClipTimer <= uiDiff) {
				...
				m_uiWingClipTimer = urand(4 * IN_MILLISECONDS,
						8 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_boomkin() :
			CreatureScript("mob_toc_boomkin") {
	}
	...
	struct mob_toc_boomkinAI: public boss_faction_championsAI {
		mob_toc_boomkinAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_RANGED) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiBarkskinTimer = urand(5 * IN_MILLISECONDS,
					120 * IN_MILLISECONDS);
			m_uiCycloneTimer = urand(5 * IN_MILLISECONDS, 40 * IN_MILLISECONDS);
			m_uiEntanglingRootsTimer = urand(5 * IN_MILLISECONDS,
					40 * IN_MILLISECONDS);
			m_uiFaerieFireTimer = urand(10 * IN_MILLISECONDS,
					40 * IN_MILLISECONDS);
			m_uiCommonTimer = urand(15 * IN_MILLISECONDS, 30 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiCycloneTimer <= uiDiff) {
				...
				m_uiCycloneTimer = urand(5 * IN_MILLISECONDS,
						40 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_warrior() :
			CreatureScript("mob_toc_warrior") {
	}
	...
	struct mob_toc_warriorAI: public boss_faction_championsAI {
		mob_toc_warriorAI(Creature *pCreature) :
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiBladestormTimer = urand(20 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
			m_uiIntimidatingShoutTimer = urand(10 * IN_MILLISECONDS,
					60 * IN_MILLISECONDS);
			m_uiMortalStrikeTimer = urand(6 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
			m_uiSunderArmorTimer = urand(5 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
			m_uiChargeTimer = urand(3 * IN_MILLISECONDS, 25 * IN_MILLISECONDS);
			m_uiRetaliationTimer = urand(30 * IN_MILLISECONDS,
					60 * IN_MILLISECONDS);
			m_uiOverpowerTimer = urand(30 * IN_MILLISECONDS,
					90 * IN_MILLISECONDS);
			m_uiShatteringThrowTimer = urand(10 * IN_MILLISECONDS,
					25 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiChargeTimer <= uiDiff) {
				...
				m_uiChargeTimer = urand(3 * IN_MILLISECONDS,
						25 * IN_MILLISECONDS);
			} else
			...
			if (m_uiDisarmTimer <= uiDiff) {
				...
				m_uiDisarmTimer = urand(20 * IN_MILLISECONDS,
						80 * IN_MILLISECONDS);
			} else
...
public:
	...
	struct mob_toc_dkAI: public boss_faction_championsAI {
		mob_toc_dkAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_MELEE) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiIceboundFortitudeTimer = urand(5 * IN_MILLISECONDS,
					90 * IN_MILLISECONDS);
			m_uiChainsOfIceTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiDeathCoilTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiStrangulateTimer = urand(10 * IN_MILLISECONDS,
					90 * IN_MILLISECONDS);
			m_uiFrostStrikeTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
			m_uiIcyTouchTimer = urand(8 * IN_MILLISECONDS,
					12 * IN_MILLISECONDS);
			m_uiDeathGripTimer = urand(5 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
...
public:
	...
	struct mob_toc_rogueAI: public boss_faction_championsAI {
		mob_toc_rogueAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_MELEE) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiFanOfKnivesTimer = urand(8 * IN_MILLISECONDS,
					10 * IN_MILLISECONDS);
			m_uiHemorrhageTimer = urand(5 * IN_MILLISECONDS,
					8 * IN_MILLISECONDS);
			m_uiEviscerateTimer = urand(15 * IN_MILLISECONDS,
					20 * IN_MILLISECONDS);
			m_uiShadowstepTimer = urand(10 * IN_MILLISECONDS,
					80 * IN_MILLISECONDS);
			m_uiBlindTimer = urand(7 * IN_MILLISECONDS, 8 * IN_MILLISECONDS);
			m_uiCloakTimer = urand(20 * IN_MILLISECONDS, 120 * IN_MILLISECONDS);
			m_uiBladeFlurryTimer = urand(12 * IN_MILLISECONDS,
					120 * IN_MILLISECONDS);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiBlindTimer <= uiDiff) {
				...
				m_uiBlindTimer = urand(7 * IN_MILLISECONDS,
						8 * IN_MILLISECONDS);
			} else
			...
			if (m_uiCloakTimer <= uiDiff) {
				...
				m_uiCloakTimer = urand(20 * IN_MILLISECONDS,
						120 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_enh_shaman() :
			CreatureScript("mob_toc_enh_shaman") {
	}
	...
	struct mob_toc_enh_shamanAI: public boss_faction_championsAI {
		mob_toc_enh_shamanAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_MELEE), Summons(me) {
		}
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiHeroismOrBloodlustTimer = urand(25 * IN_MILLISECONDS,
					60 * IN_MILLISECONDS);
			m_uiEarthShockTimer = urand(5 * IN_MILLISECONDS,
					8 * IN_MILLISECONDS);
			m_uiStormstrikeTimer = urand(5 * IN_MILLISECONDS,
					90 * IN_MILLISECONDS);
			m_uiLavaLashTimer = urand(5 * IN_MILLISECONDS, 8 * IN_MILLISECONDS);
			m_uiDeployTotemTimer = urand(1 * IN_MILLISECONDS,
					3 * IN_MILLISECONDS);
			m_uiTotemCount = 0;
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiLavaLashTimer <= uiDiff) {
				...
				m_uiLavaLashTimer = urand(5 * IN_MILLISECONDS,
						8 * IN_MILLISECONDS);
			} else
			...
			if (m_uiDeployTotemTimer <= uiDiff) {
				if (m_uiTotemCount < 4
						|| me->GetDistance2d(m_fTotemOldCenterX,
								m_fTotemOldCenterY) > 20.0f)
					DeployTotem();
				m_uiDeployTotemTimer = urand(1 * IN_MILLISECONDS,
						3 * IN_MILLISECONDS);
			} else
...
public:
	mob_toc_retro_paladin() :
			CreatureScript("mob_toc_retro_paladin") {
	}
	...
	struct mob_toc_retro_paladinAI: public boss_faction_championsAI {
		mob_toc_retro_paladinAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_MELEE) {
		}
		...
		void Reset() {
			...
			m_uiCrusaderStrikeTimer = urand(6 * IN_MILLISECONDS,
					18 * IN_MILLISECONDS);
			...
			m_uiJudgementOfCommandTimer = urand(8 * IN_MILLISECONDS,
					15 * IN_MILLISECONDS);
...
enum eWarlockPetSpells {
	SPELL_DEVOUR_MAGIC = 67518, SPELL_SPELL_LOCK = 67519,
};

class mob_toc_pet_warlock: public CreatureScript {
public:
	mob_toc_pet_warlock() :
			CreatureScript("mob_toc_pet_warlock") {
	}
	...
	struct mob_toc_pet_warlockAI: public boss_faction_championsAI {
		mob_toc_pet_warlockAI(Creature *pCreature) :
		...
		void Reset() {
			boss_faction_championsAI::Reset();
			m_uiDevourMagicTimer = urand(15 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
			m_uiSpellLockTimer = urand(15 * IN_MILLISECONDS,
					30 * IN_MILLISECONDS);
		}
...
public:
	mob_toc_pet_hunter() :
			CreatureScript("mob_toc_pet_hunter") {
	}
	...
	struct mob_toc_pet_hunterAI: public boss_faction_championsAI {
		mob_toc_pet_hunterAI(Creature *pCreature) :
				boss_faction_championsAI(pCreature, AI_PET) {
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			if (m_uiClawTimer <= uiDiff) {
				...
				m_uiClawTimer = urand(5 * IN_MILLISECONDS,
						10 * IN_MILLISECONDS);
			} else
