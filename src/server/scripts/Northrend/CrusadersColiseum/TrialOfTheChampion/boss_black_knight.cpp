...
enum eModels {
	MODEL_SKELETON = 29846, MODEL_GHOST = 21300
};
...
enum ePhases {
	PHASE_UNDEAD = 1, PHASE_SKELETON = 2, PHASE_GHOST = 3
};
...
public:
	boss_black_knight() :
			CreatureScript("boss_black_knight") {
	}
	...
	struct boss_black_knightAI: public BossAI {
		boss_black_knightAI(Creature* pCreature) :
				BossAI(pCreature, BOSS_BLACK_KNIGHT) {
		...
		void EnterEvadeMode() {
			...
			if (bEventInBattle) {
				me->GetMotionMaster()->MovePoint(1, 743.396f, 635.411f,
						411.575f);
				me->setFaction(14);
		...
		void UpdateAI(const uint32 uiDiff) {
			...
			case PHASE_GHOST: {
				if (uiDeathBiteTimer <= uiDiff) {
					SetEquipmentSlots(false, EQUIP_UNEQUIP, EQUIP_NO_CHANGE,
							EQUIP_NO_CHANGE);
		...
		void EnterCombat(Unit* pWho) {
			...
			SetEquipmentSlots(false, EQUIP_SWORD, EQUIP_NO_CHANGE,
					EQUIP_NO_CHANGE);
			if (GameObject* pGO = GameObject::GetGameObject(*me, instance->GetData64(DATA_MAIN_GATE1)))
...
public:
	npc_risen_ghoul() :
			CreatureScript("npc_risen_ghoul") {
	}
	...
	struct npc_risen_ghoulAI: public ScriptedAI {
		npc_risen_ghoulAI(Creature* pCreature) :
...
public:
	...
	struct npc_black_knight_skeletal_gryphonAI: public npc_escortAI {
		...
		void WaypointReached(uint32 uiPointId) {
			...
			case 13:
				me->SetUnitMovementFlags(MOVEMENTFLAG_WALKING);
				me->SummonCreature(VEHICLE_GR, 744.841f, 634.505f, 411.575f,
						2.79f);
				break;
			}
		}
