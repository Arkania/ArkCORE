...
enum eEnums {
	SAY_START_1 = -1999939, SAY_START_2 = -1999937
};
...
bool GrandChampionsOutVehicle(Creature* me) {
	...
	Creature* pGrandChampion1 = Unit::GetCreature(*me,
			pInstance->GetData64(DATA_GRAND_CHAMPION_1));
	Creature* pGrandChampion2 = Unit::GetCreature(*me,
			pInstance->GetData64(DATA_GRAND_CHAMPION_2));
	Creature* pGrandChampion3 = Unit::GetCreature(*me,
			pInstance->GetData64(DATA_GRAND_CHAMPION_3));

	if (pGrandChampion1 && pGrandChampion2 && pGrandChampion3) {
		if (!pGrandChampion1->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT)
				&& !pGrandChampion2->HasUnitMovementFlag(
						MOVEMENTFLAG_ONTRANSPORT)
				&& !pGrandChampion3->HasUnitMovementFlag(
						MOVEMENTFLAG_ONTRANSPORT))
			return true;
	}
...
public:
	generic_vehicleAI_toc5() :
			CreatureScript("generic_vehicleAI_toc5") {
	}
	...
	struct generic_vehicleAI_toc5AI: public npc_escortAI {
		generic_vehicleAI_toc5AI(Creature* pCreature) :
				npc_escortAI(pCreature), vehicle(pCreature->GetVehicleKit()) {
		...
		void WaypointReached(uint32 i) {
			...
			case 2:
				if (pInstance && uiWaypointPath == 3 || uiWaypointPath == 2)
					pInstance->SetData(DATA_MOVEMENT_DONE,
							pInstance->GetData(DATA_MOVEMENT_DONE) + 1);
				break;
			case 3:
				if (pInstance)
					pInstance->SetData(DATA_MOVEMENT_DONE,
					...
					if (me->GetMap()->IsDungeon() && !players.isEmpty()) {
						for (Map::PlayerList::const_iterator itr =
							...
							if (pPlayer && !pPlayer->isGameMaster()
									&& me->IsInRange(pPlayer, 10.0f, 30.0f,
											false)) {
								pPassenger->CastSpell(pPlayer,
										SPELL_SHIELD_BREAKER, true);
								break;
							}
						}
					}
...
public:
	boss_warrior_toc5() :
			CreatureScript("boss_warrior_toc5") {
	}
	...
	struct boss_warrior_toc5AI: public ScriptedAI {
		boss_warrior_toc5AI(Creature* pCreature) :
			...
			if (uiInterceptTimer <= uiDiff) {
				...
				if (me->GetMap()->IsDungeon() && !players.isEmpty()) {
					for (Map::PlayerList::const_iterator itr = players.begin();
						...
						Player* pPlayer = itr->getSource();
						if (pPlayer && !pPlayer->isGameMaster()
								&& me->IsInRange(pPlayer, 8.0f, 25.0f, false)) {
							DoResetThreat();
...
public:
	...
	struct boss_mage_toc5AI: public ScriptedAI {
		boss_mage_toc5AI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			pInstance = pCreature->GetInstanceScript();
...
public:
	boss_shaman_toc5() :
			CreatureScript("boss_shaman_toc5") {
	}
	...
	struct boss_shaman_toc5AI: public ScriptedAI {
		boss_shaman_toc5AI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			pInstance = pCreature->GetInstanceScript();
...
public:
	boss_hunter_toc5() :
			CreatureScript("boss_hunter_toc5") {
	}
	...
	struct boss_hunter_toc5AI: public ScriptedAI {
		boss_hunter_toc5AI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			pInstance = pCreature->GetInstanceScript();
				...
				if (pTarget && me->IsInRange(pTarget, 5.0f, 30.0f, false)) {
					...
					if (me->GetMap()->IsDungeon() && !players.isEmpty()) {
						for (Map::PlayerList::const_iterator itr =
							...
							Player* pPlayer = itr->getSource();
							if (pPlayer && !pPlayer->isGameMaster()
									&& me->IsInRange(pPlayer, 5.0f, 30.0f,
											false)) {
								DoCast(pTarget, SPELL_MULTI_SHOT);
								break;
							}
...
public:
	boss_rouge_toc5() :
			CreatureScript("boss_rouge_toc5") {
	}
	...
	struct boss_rouge_toc5AI: public ScriptedAI {
		boss_rouge_toc5AI(Creature* pCreature) :
				ScriptedAI(pCreature) {
			pInstance = pCreature->GetInstanceScript();
