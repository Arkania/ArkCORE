...
public:
	...
	struct instance_trial_of_the_champion_InstanceMapScript: public InstanceScript {
				...
				if (uiData == NOT_STARTED) {
					...
					if (Creature* pBlackKnight = instance->GetCreature(uiBlackKnightGUID)) {
						if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID)) {
							if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID))
								pBlackKnight->CastSpell(pAnnouncer,
										SPELL_DEATH_RESPITE, true);
						}
					}
				}
				break;
			...
			case BOSS_GRAND_CHAMPIONS:
				...
				if (uiData == IN_PROGRESS) {
					for (std::list<uint64>::const_iterator itr =
							VehicleList.begin(); itr != VehicleList.end();
							++itr)
					...
					if (uiGrandChampionsDeaths == 3) {
						if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID)) {
							...
							pAnnouncer->SummonGameObject(
									instance->IsHeroic() ?
											GO_CHAMPIONS_LOOT_H :
											GO_CHAMPIONS_LOOT, 746.59f, 618.49f,
									411.09f, 1.42f, 0, 0, 0, 0, 90000000);
						}
					}
				}
				break;
			case DATA_ARGENT_SOLDIER_DEFEATED:
				...
				if (uiArgentSoldierDeaths == 9) {
					...
					if (Creature* pBoss = instance->GetCreature(uiArgentChampionGUID)) {
						pBoss->setFaction(14);
						pBoss->RemoveFlag(UNIT_FIELD_FLAGS,
								UNIT_FLAG_NON_ATTACKABLE);
						pBoss->RemoveFlag(UNIT_FIELD_FLAGS,
								UNIT_FLAG_NOT_SELECTABLE);
			...
			case BOSS_ARGENT_CHALLENGE_E:
				...
				if (uiData == IN_PROGRESS) {
					...
					if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID)) {
						...
						pAnnouncer->SetFlag(UNIT_NPC_FLAGS,
								UNIT_NPC_FLAG_GOSSIP);
						pAnnouncer->GetMotionMaster()->MovePoint(0, 740.755f,
								636.509f, 411.575f);
						pAnnouncer->SummonGameObject(
								instance->IsHeroic() ?
										GO_EADRIC_LOOT_H : GO_EADRIC_LOOT,
								746.59f, 618.49f, 411.09f, 1.42f, 0, 0, 0, 0,
								90000000);
					}
				break;
			case BOSS_ARGENT_CHALLENGE_P:
				...
				if (uiData == IN_PROGRESS) {
					...
					if (Creature* pAnnouncer = instance->GetCreature(uiAnnouncerGUID)) {
						...
						pAnnouncer->SummonGameObject(
								instance->IsHeroic() ?
										GO_PALETRESS_LOOT_H : GO_PALETRESS_LOOT,
								746.59f, 618.49f, 411.09f, 1.42f, 0, 0, 0, 0,
								90000000);
					}
				break;
		...
		uint32 GetData(uint32 uiData) {
			...
			case DATA_KNIGHT:
				return uiBlackKnightEvent2;
			case DATA_BLACK_KNIGHT:
				return uiBlackKnightEvent;
			case DATA_MOVEMENT_DONE:
		...
		std::string GetSaveData() {
			...
			saveStream << "T C" << m_auiEncounter[0] << " " << m_auiEncounter[1]
					<< " " << m_auiEncounter[2] << " " << m_auiEncounter[3]
					<< " " << uiGrandChampionsDeaths << " " << uiMovementDone;
