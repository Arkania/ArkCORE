...
public:
	...
	struct npc_anstartAI: public ScriptedAI {
		npc_anstartAI(Creature *pCreature) :
		...
		void MoveInLineOfSight(Unit* pWho) {
			...
			if (Phase == IDLE && pWho->isTargetableForAttack()
				...
				if (Creature* pGarrosh = me->SummonCreature(CREATURE_GARROSH, 685.7f, 621.134f, 435.396f, 6.259f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000)) {
					...
					pGarrosh->SetFlag(UNIT_FIELD_FLAGS,
				...
				if (Creature* pHighlord = me->SummonCreature(CREATURE_HIGHLORD, 746.482f, 556.857f, 435.396f, 1.5898f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000)) {
					...
					pHighlord->SetFlag(UNIT_FIELD_FLAGS,
							UNIT_FLAG_NON_ATTACKABLE);
				}
			}
		}
			...
			if (uiIntroTimer <= diff) {
				...
				case 9:
					if (Creature* pAnnouncertoc5 = me->SummonCreature(CREATURE_ANNOUNCER, 746.626f, 618.54f, 411.09f, 4.63158f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000)) {
						me->DisappearAndDie();
						pAnnouncertoc5->SetFlag(UNIT_FIELD_FLAGS,
								UNIT_FLAG_NON_ATTACKABLE);
						pAnnouncertoc5->SetFlag(UNIT_NPC_FLAGS,
	...
	struct npc_announcer_toc5AI: public ScriptedAI {
		...
		void SetData(uint32 uiType, uint32 uiData) {
			...
			case DATA_IN_POSITION: //movement done.
				...
				me->GetMotionMaster()->MovePoint(1, 735.898f, 651.961f,
						411.93f);
		...
		void DoSummonGrandChampion(uint32 uiBoss) {
			...
			if (Creature* pBoss = me->SummonCreature(VEHICLE_TO_SUMMON1, SpawnPosition)) {
				...
				case 1: {
					...
					if (pInstance) {
						pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_1,
								uiVehicle1GUID);
						pInstance->SetData64(DATA_GRAND_CHAMPION_1,
				...
				case 2: {
					...
					if (pInstance) {
						pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_2,
								uiVehicle2GUID);
						pInstance->SetData64(DATA_GRAND_CHAMPION_2,
								uiGrandChampionBoss2);
					}
				...
				case 3: {
					...
					if (pInstance) {
						pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_3,
								uiVehicle3GUID);
						pInstance->SetData64(DATA_GRAND_CHAMPION_3,
								uiGrandChampionBoss3);
					}
				...
				for (uint8 i = 0; i < 3; ++i) {
					if (Creature* pAdd = me->SummonCreature(VEHICLE_TO_SUMMON2, SpawnPosition, TEMPSUMMON_CORPSE_DESPAWN)) {
						...
						switch (i) {
						case 0:
							pAdd->GetMotionMaster()->MoveFollow(pBoss, 2.0f,
									M_PI);
							break;
						case 1:
							pAdd->GetMotionMaster()->MoveFollow(pBoss, 2.0f,
		...
		void SetGrandChampionsForEncounter() {
			...
			while (uiSecondBoss == uiFirstBoss || uiThirdBoss == uiFirstBoss
					|| uiThirdBoss == uiSecondBoss) {
		...
		void StartEncounter() {
			...
			if (pInstance->GetData(BOSS_BLACK_KNIGHT) == NOT_STARTED) {
				if (pInstance->GetData(BOSS_ARGENT_CHALLENGE_E) == NOT_STARTED
						&& pInstance->GetData(BOSS_ARGENT_CHALLENGE_P)
								== NOT_STARTED) {
				...
				if (pInstance->GetData(BOSS_GRAND_CHAMPIONS) == DONE
					...
					if (Unit* pBlackKnight = me->SummonCreature(VEHICLE_BLACK_KNIGHT, 801.369507f, 640.574280f, 469.314362f, 3.97124f, TEMPSUMMON_DEAD_DESPAWN, 180000)) {
						...
						pBlackKnight->SetFlag(UNIT_FIELD_FLAGS,
								UNIT_FLAG_NON_ATTACKABLE);
						pBlackKnight->SetUInt64Value(UNIT_FIELD_TARGET,
								me->GetGUID());
		...
		void AggroAllPlayers(Creature* pTemp) {
			...
			for (Map::PlayerList::const_iterator i = PlList.begin();
				...
				if (Player* pPlayer = i->getSource()) {
					...
					if (pPlayer->isAlive()) {
						pTemp->SetHomePosition(me->GetPositionX(),
