...
enum eAgnetta {
	QUEST_ITS_THAT_YOUR_GOBLIN = 12969, FACTION_HOSTILE_AT1 = 45
};

class npc_agnetta_tyrsdottar: public CreatureScript {
public:
	npc_agnetta_tyrsdottar() :
			CreatureScript("npc_agnetta_tyrsdottar") {
	}

	struct npc_agnetta_tyrsdottarAI: public ScriptedAI {
		npc_agnetta_tyrsdottarAI(Creature* pCreature) :
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		if (pPlayer->GetQuestStatus(QUEST_ITS_THAT_YOUR_GOBLIN)
...
public:
	npc_frostborn_scout() :
			CreatureScript("npc_frostborn_scout") {
	}

	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		if (pPlayer->GetQuestStatus(QUEST_MISSING_SCOUTS)
				== QUEST_STATUS_INCOMPLETE) {
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
			pPlayer->PlayerTalkClass->SendGossipMenu(13611,
					pCreature->GetGUID());
		}
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
		...
		case GOSSIP_ACTION_INFO_DEF + 1:
			...
			pPlayer->PlayerTalkClass->SendGossipMenu(13612,
					pCreature->GetGUID());
			break;
		case GOSSIP_ACTION_INFO_DEF + 2:
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM3, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+3);
			pPlayer->PlayerTalkClass->SendGossipMenu(13613,
					pCreature->GetGUID());
			break;
		case GOSSIP_ACTION_INFO_DEF + 3:
			pPlayer->PlayerTalkClass->SendGossipMenu(13614,
					pCreature->GetGUID());
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_SIBLING_RIVALRY)
				== QUEST_STATUS_INCOMPLETE) {
...
public:
	npc_goblin_prisoner() :
			CreatureScript("npc_goblin_prisoner") {
	}

	struct npc_goblin_prisonerAI: public ScriptedAI {
		npc_goblin_prisonerAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
enum eVictoriousChallenger {
	QUEST_TAKING_ALL_CHALLENGERS = 12971, QUEST_DEFENDING_YOUR_TITLE = 13423,

	SPELL_SUNDER_ARMOR = 11971, SPELL_REND_VC = 11977
};

class npc_victorious_challenger: public CreatureScript {
public:
	npc_victorious_challenger() :
			CreatureScript("npc_victorious_challenger") {
	}

	struct npc_victorious_challengerAI: public ScriptedAI {
		npc_victorious_challengerAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_TAKING_ALL_CHALLENGERS)
				== QUEST_STATUS_INCOMPLETE
				|| pPlayer->GetQuestStatus(QUEST_DEFENDING_YOUR_TITLE)
						== QUEST_STATUS_INCOMPLETE) {
...
public:
	npc_loklira_crone() :
			CreatureScript("npc_loklira_crone") {
	}
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_HYLDSMEET)
				== QUEST_STATUS_INCOMPLETE) {
...
public:
	...
	struct npc_injured_goblinAI: public npc_escortAI {
		npc_injured_goblinAI(Creature* pCreature) :
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_BITTER_DEPARTURE)
				== QUEST_STATUS_INCOMPLETE) {
			pPlayer->ADD_GOSSIP_ITEM(0, GOSSIP_ITEM_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
			pPlayer->PlayerTalkClass->SendGossipMenu(9999999,
					pCreature->GetGUID());
		} else
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
			uint32 /*uiSender*/, uint32 uiAction) {
		pPlayer->PlayerTalkClass->ClearMenus();
		npc_escortAI* pEscortAI =
				CAST_AI(npc_injured_goblin::npc_injured_goblinAI, pCreature->AI());
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pCreature->isVendor())
			if (pPlayer->HasSpell(SPELL_MECHANO_HOG)
...
public:
	...
	struct npc_brunnhildar_prisonerAI: public ScriptedAI {
		npc_brunnhildar_prisonerAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
		...
		void MoveInLineOfSight(Unit *unit) {
			...
			if (!me->IsOnVehicle(drake) && !me->HasAura(SPELL_ICE_PRISON)) {
				if (unit->IsVehicle() && me->IsWithinDist(unit, 25.0f, true)
						&& unit->ToCreature()
						&& unit->ToCreature()->GetEntry() == 29709) {
					uint8 seat = unit->GetVehicleKit()->GetNextEmptySeat(0,
							true);
					if (seat <= 0)
						return;
			...
			if (unit->ToCreature() && me->IsOnVehicle(drake)) {
				if (unit->ToCreature()->GetEntry() == NPC_QUEST_GIVER
						&& me->IsWithinDist(unit, 15.0f, false)) {
