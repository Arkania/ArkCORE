...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_CHILDREN_OF_URSOC)
				== QUEST_STATUS_INCOMPLETE
				|| pPlayer->GetQuestStatus(QUEST_THE_BEAR_GODS_OFFSPRING)
						== QUEST_STATUS_INCOMPLETE) {
			switch (pCreature->GetEntry()) {
			case NPC_ORSONN:
				if (!pPlayer->GetReqKillOrCastCurrentCount(
						QUEST_CHILDREN_OF_URSOC, NPC_ORSONN_CREDIT)
						|| !pPlayer->GetReqKillOrCastCurrentCount(
								QUEST_THE_BEAR_GODS_OFFSPRING,
								NPC_ORSONN_CREDIT)) {
			...
			case NPC_KODIAN:
				if (!pPlayer->GetReqKillOrCastCurrentCount(
						QUEST_CHILDREN_OF_URSOC, NPC_KODIAN_CREDIT)
						|| !pPlayer->GetReqKillOrCastCurrentCount(
								QUEST_THE_BEAR_GODS_OFFSPRING,
								NPC_KODIAN_CREDIT)) {
...
enum eFloppy {
	NPC_MRFLOPPY = 26589, NPC_HUNGRY_WORG = 26586, NPC_RAVENOUS_WORG = 26590, //RWORG
	NPC_EMILY = 26588,
...
public:
	...
	struct npc_emilyAI: public npc_escortAI {
		npc_emilyAI(Creature* pCreature) :
		...
		void WaypointReached(uint32 i) {
			...
			case 10:
				if (Unit::GetCreature(*me, MrfloppyGUID)) {
					...
					me->SummonCreature(NPC_HUNGRY_WORG, me->GetPositionX() + 5,
							me->GetPositionY() + 2, me->GetPositionZ() + 1,
							3.229f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 120000);
				}
				break;
			case 11:
				if (Creature *Mrfloppy = Unit::GetCreature(*me, MrfloppyGUID))
					Mrfloppy->GetMotionMaster()->MoveFollow(me, PET_FOLLOW_DIST,
							PET_FOLLOW_ANGLE);
				break;
			case 17:
				if (Creature *Mrfloppy = Unit::GetCreature(*me, MrfloppyGUID))
			...
			case 18:
				if (Creature *Mrfloppy = Unit::GetCreature(*me, MrfloppyGUID)) {
					if (Creature *RWORG = Unit::GetCreature(*me, RWORGGUID))
						RWORG->GetMotionMaster()->MovePoint(0,
			...
			case 21:
				if (Creature *Mrfloppy = Unit::GetCreature(*me, MrfloppyGUID)) {
					if (Creature *RWORG = Unit::GetCreature(*me, RWORGGUID)) {
						...
						RWORG->setFaction(14);
						RWORG->GetMotionMaster()->MovePoint(0,
								RWORG->GetPositionX() + 10,
								RWORG->GetPositionY() + 80,
								RWORG->GetPositionZ());
			...
			case 22:
				if (Creature *Mrfloppy = Unit::GetCreature(*me, MrfloppyGUID)) {
					if (Mrfloppy->isDead()) {
						...
						Mrfloppy->GetMotionMaster()->MoveFollow(me,
	...
	bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
		...
		if (quest->GetQuestId() == QUEST_PERILOUS_ADVENTURE) {
			...
			if (Creature* Mrfloppy = GetClosestCreatureWithEntry(pCreature, NPC_MRFLOPPY, 180.0f)) {
				Mrfloppy->GetMotionMaster()->MoveFollow(pCreature,
						PET_FOLLOW_DIST, PET_FOLLOW_ANGLE);
			}
...
public:
	...
	struct npc_outhouse_bunnyAI: public ScriptedAI {
		npc_outhouse_bunnyAI(Creature* pCreature) :
...
public:
	...
	struct npc_tallhorn_stagAI: public ScriptedAI {
		npc_tallhorn_stagAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
public:
	...
	struct npc_amberpine_woodsmanAI: public ScriptedAI {
		npc_amberpine_woodsmanAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
enum eSkirmisher {
	SPELL_RENEW_SKIRMISHER = 48812, CREDIT_NPC = 27466,
...
public:
	...
	struct npc_wounded_skirmisherAI: public ScriptedAI {
		npc_wounded_skirmisherAI(Creature *c) :
				ScriptedAI(c) {
		}
		...
		void SpellHit(Unit *caster, const SpellEntry *spell) {
			if (spell->Id == SPELL_RENEW_SKIRMISHER
					...
					&& caster->ToPlayer()->GetQuestStatus(12288)
							== QUEST_STATUS_INCOMPLETE) {
				caster->ToPlayer()->KilledMonsterCredit(CREDIT_NPC, 0);
				DoScriptText(RAND(RANDOM_SAY_1, RANDOM_SAY_2, RANDOM_SAY_3),
						caster);
				if (me->IsStandState())
					me->GetMotionMaster()->MovePoint(1, me->GetPositionX() + 7,
							me->GetPositionY() + 7, me->GetPositionZ());
				else {
...
enum eSentry {
	QUEST_OR_MAYBE_WE_DONT_A = 12138, QUEST_OR_MAYBE_WE_DONT_H = 12198,

	NPC_LIGHTNING_SENTRY = 26407, NPC_WAR_GOLEM = 27017,
...
public:
	...
	struct npc_lightning_sentryAI: public ScriptedAI {
		npc_lightning_sentryAI(Creature *pCreature) :
				ScriptedAI(pCreature) {
		}
		...
		void JustDied(Unit * pKiller) {
			if (pKiller->ToPlayer()
					&& pKiller->ToPlayer()->GetTypeId() == TYPEID_PLAYER) {
				if (me->FindNearestCreature(NPC_WAR_GOLEM, 10.0f, true)) {
					if (pKiller->ToPlayer()->GetQuestStatus(
							QUEST_OR_MAYBE_WE_DONT_A) == QUEST_STATUS_INCOMPLETE
							|| pKiller->ToPlayer()->GetQuestStatus(
									QUEST_OR_MAYBE_WE_DONT_H)
									== QUEST_STATUS_INCOMPLETE)
						pKiller->ToPlayer()->KilledMonsterCredit(NPC_WAR_GOLEM,
								0);
				}
			}
		}
...
public:
	...
	struct npc_venture_co_stragglerAI: public ScriptedAI {
		npc_venture_co_stragglerAI(Creature *pCreature) :
				ScriptedAI(pCreature) {
		}
		...
		void UpdateAI(const uint32 uiDiff) {
			if (uiRunAwayTimer <= uiDiff) {
				if (Player *pPlayer = Unit::GetPlayer(*me, uiPlayerGUID)) {
					switch (uiTimer) {
					case 0:
						if (pPlayer->GetQuestStatus(QUEST_SMOKE_EM_OUT_A)
								== QUEST_STATUS_INCOMPLETE
								|| pPlayer->GetQuestStatus(QUEST_SMOKE_EM_OUT_H)
										== QUEST_STATUS_INCOMPLETE)
							pPlayer->KilledMonsterCredit(
									NPC_VENTURE_CO_STABLES_KC, 0);
						me->GetMotionMaster()->MovePoint(0,
								me->GetPositionX() - 7, me->GetPositionY() + 7,
								me->GetPositionZ());
						uiRunAwayTimer = 2500;
						++uiTimer;
						break;
					case 1:
						DoScriptText(
								RAND(SAY_SEO1, SAY_SEO2, SAY_SEO3, SAY_SEO4,
										SAY_SEO5), me);
						me->GetMotionMaster()->MovePoint(0,
								me->GetPositionX() - 7, me->GetPositionY() - 5,
								me->GetPositionZ());
						uiRunAwayTimer = 2500;
						++uiTimer;
						break;
					case 2:
						me->GetMotionMaster()->MovePoint(0,
								me->GetPositionX() - 5, me->GetPositionY() - 5,
								me->GetPositionZ());
						uiRunAwayTimer = 2500;
						++uiTimer;
						break;
