...
public:
	...
	struct npc_drakuru_shacklesAI: public ScriptedAI {
		npc_drakuru_shacklesAI(Creature* pCreature) :
...
enum eRageclaw {
	SPELL_UNSHACKLED = 55085, SPELL_KNEEL = 39656
};
...
public:
	...
	struct npc_captured_rageclawAI: public ScriptedAI {
		npc_captured_rageclawAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
enum eGymer {
	QUEST_STORM_KING_VENGEANCE = 12919, SPELL_GYMER = 55568
};

class npc_gymer: public CreatureScript {
public:
	npc_gymer() :
			CreatureScript("npc_gymer") {
	}
...
static BossAndAdd Boss[] = { { NPC_GARGORAL, NPC_FIEND_WATER,
		SPELL_CRASHING_WAVE, SPELL_ORB_OF_WATER }, { NPC_AZ_BARIN,
		NPC_FIEND_AIR, SPELL_BLAST_OF_AIR, SPELL_ORB_OF_STORMS }, {
...
const Position SpawnPosition[] = { { 5754.692f, -2939.46f, 286.276123f,
		5.156380f }, // stinkbeard || orinoko || korrak
		{ 5762.054199f, -2954.385010f, 273.826955f, 5.108289f }, //yggdras
		{ 5776.855f, -2989.77979f, 272.96814f, 5.194f } // elementals
};

const Position AddSpawnPosition[] = { { 5722.487f, -3010.75f, 312.751648f,
		0.478f }, // caster location
		{ 5724.983f, -2969.89551f, 286.359619f, 0.478f }, { 5733.76025f,
				-3000.34644f, 286.359619f, 0.478f }, { 5739.8125f, -2981.524f,
				290.7671f, 0.478f }, // caster location
		{ 5742.101f, -2950.75586f, 286.2643f, 5.21f }, { 5743.305f,
				-3011.29736f, 290.7671f, 0.478f }, // caster location
		{ 5744.417f, -3025.528f, 286.35965f, 0.478f }, { 5763.189f,
				-3029.67529f, 290.7671f, 0.478f }, { 5769.401f, -2935.121f,
				286.335754f, 5.21f }, { 5793.061f, -2934.593f, 286.359619f,
				3.53f }, { 5797.32129f, -2955.26855f, 290.7671f, 3.53f }, // caster location
		{ 5813.94531f, -2956.74683f, 286.359619f, 3.53f }, { 5816.85547f,
				-2974.476f, 290.7671f, 3.53f }, // caster location
		{ 5820.30859f, -3002.83716f, 290.7671f, 3.53f }, // caster location
		{ 5828.50244f, -2981.737f, 286.359619f, 3.53f }, { 5828.899f,
				-2960.15479f, 312.751648f, 3.53f }, // caster location
		};

class npc_gurgthock: public CreatureScript {
public:
	npc_gurgthock() :
			CreatureScript("npc_gurgthock") {
	}

	struct npc_gurgthockAI: public ScriptedAI {
		npc_gurgthockAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
				...
				if (uiTimer <= uiDiff) {
					...
					case 2:
						if (Creature* pSummon = Unit::GetCreature(*me, SummonGUID))
							pSummon->GetMotionMaster()->MoveJump(5776.319824f,
									-2981.005371f, 273.100037f, 10.0f, 20.0f);
						uiPhase = 0;
						SummonGUID = 0;
						break;
					case 3:
					...
					case 6: {
						...
						std::string sText =
								("The grand Amphitheater of Anguish awaits, "
										+ std::string(pPlayer->GetName())
										+ ". Remember, once a battle starts you have to stay in the area. WIN OR DIE!");
					...
					case 7: {
						...
						std::string sText =
								("Prepare to make you stand, "
										+ std::string(pPlayer->GetName())
										+ "! Get in the Amphitheater and stand ready! Remember, you and your opponent must stay in the arena at all times or you will be disqualified!");
						me->MonsterSay(sText.c_str(), LANG_UNIVERSAL, 0);
						uiTimer = 3000;
						uiPhase = 8;
					}
					...
					case 9: {
						...
						std::string sText =
								("Here we are once again, ladies and gentlemen. The epic struggle between life and death in the Amphitheater of Anguish! For this round we have "
										+ std::string(pPlayer->GetName())
										+ " versus the hulking jormungar, Yg... Yggd? Yggdoze? Who comes up with these names?! "
										+ std::string(pPlayer->GetName())
										+ " versus big worm!");
						...
						break;
					case 10:
						me->SummonCreature(NPC_YGGDRAS, SpawnPosition[1],
...
public:
	npc_orinoko_tuskbreaker() :
			CreatureScript("npc_orinoko_tuskbreaker") {
	}

	struct npc_orinoko_tuskbreakerAI: public ScriptedAI {
		npc_orinoko_tuskbreakerAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		...
		void JustDied(Unit* pKiller) {
			...
			if (pKiller->GetTypeId() == TYPEID_PLAYER)
				pKiller->GetCharmerOrOwnerPlayerOrPlayerItself()->GroupEventHappens(
						QUEST_AMPHITHEATER_ANGUISH_TUSKARRMAGEDDON, pKiller);
		}
	};
...
public:
	npc_korrak_bloodrager() :
			CreatureScript("npc_korrak_bloodrager") {
	}

	struct npc_korrak_bloodragerAI: public npc_escortAI {
		npc_korrak_bloodragerAI(Creature* pCreature) :
		...
		void JustDied(Unit* pKiller) {
			if (Player* pPlayer = pKiller->GetCharmerOrOwnerPlayerOrPlayerItself())
				pPlayer->GroupEventHappens(
						QUEST_AMPHITHEATER_ANGUISH_KORRAK_BLOODRAGER, pKiller);
		}
	};
...
public:
	npc_yggdras() :
			CreatureScript("npc_yggdras") {
	}

	struct npc_yggdrasAI: public ScriptedAI {
		npc_yggdrasAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
		...
		void JustDied(Unit* pKiller) {
			if (Unit* pSummoner = me->ToTempSummon()->GetSummoner()) {
				std::string sText = (std::string(pKiller->GetName())
						+ " has defeated Yg.. Yggg-really big worm!");
				pSummoner->MonsterYell(sText.c_str(), LANG_UNIVERSAL, 0);
			}

			if (Player* pPlayer = pKiller->GetCharmerOrOwnerPlayerOrPlayerItself()) {
				pPlayer->GroupEventHappens(QUEST_AMPHITHEATER_ANGUISH_YGGDRAS_1,
						pKiller);
				pPlayer->GroupEventHappens(QUEST_AMPHITHEATER_ANGUISH_YGGDRAS_2,
						pKiller);
			}
...
public:
	npc_stinkbeard() :
			CreatureScript("npc_stinkbeard") {
	}

	struct npc_stinkbeardAI: public npc_escortAI {
		npc_stinkbeardAI(Creature* pCreature) :
				npc_escortAI(pCreature) {
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE);
			me->SetReactState(REACT_PASSIVE);
			Start(true, true, 0, NULL);
		...
		void JustDied(Unit* pKiller) {
			if (Player* pPlayer = pKiller->GetCharmerOrOwnerPlayerOrPlayerItself())
				pPlayer->GetCharmerOrOwnerPlayerOrPlayerItself()->GroupEventHappens(
						QUEST_AMPHITHEATER_ANGUISH_MAGNATAUR, pKiller);

			std::string sText =
					("And with AUTHORITY, " + std::string(pKiller->GetName())
							+ " dominates the magnataur lord! Stinkbeard's clan is gonna miss him back home in the Dragonblight!");
			me->MonsterYell(sText.c_str(), LANG_UNIVERSAL, 0);
		}
	};
...
public:
	npc_elemental_lord() :
			CreatureScript("npc_elemental_lord") {
	}

	struct npc_elemental_lordAI: public ScriptedAI {
		npc_elemental_lordAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
			...
			if (!bAddAttack && !HealthAbovePct(20)) {
				if (!SummonList.empty())
					for (std::list<uint64>::const_iterator itr =
							SummonList.begin(); itr != SummonList.end();
							++itr) {
						if (Creature* pTemp = Unit::GetCreature(*me, *itr)) {
							...
							if (pTemp->getVictim())
								pTemp->GetMotionMaster()->MoveChase(
										pTemp->getVictim());
						}
					}
		...
		void JustDied(Unit* pKiller) {
			...
			if (Player* pPlayer = pKiller->GetCharmerOrOwnerPlayerOrPlayerItself())
				pPlayer->GetCharmerOrOwnerPlayerOrPlayerItself()->GroupEventHappens(
						QUEST_AMPHITHEATER_ANGUISH_FROM_BEYOND, pKiller);

			std::string sText = (std::string(pKiller->GetName())
					+ " is victorious once more!");
...
public:
	npc_fiend_elemental() :
			CreatureScript("npc_fiend_elemental") {
	}

	struct npc_fiend_elementalAI: public ScriptedAI {
		npc_fiend_elementalAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
public:
	...
	struct npc_released_offspring_harkoaAI: public ScriptedAI {
		npc_released_offspring_harkoaAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
...
enum eCrusade_recruit {
	...
	SAY_RECRUIT_1 = -1571036, SAY_RECRUIT_2 = -1571037, SAY_RECRUIT_3 = -1571038
};
...
public:
	...
	struct npc_crusade_recruitAI: public ScriptedAI {
		npc_crusade_recruitAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
				...
				if (m_uiTimer <= uiDiff) {
					...
					case 1:
						...
						me->SetUInt32Value(UNIT_NPC_EMOTESTATE,
								EMOTE_ONESHOT_NONE);
						DoScriptText(
								RAND(SAY_RECRUIT_1, SAY_RECRUIT_2,
										SAY_RECRUIT_3), me);
						m_uiTimer = 3000;
						m_uiPhase = 2;
						break;
					case 2:
						...
						me->GetMotionMaster()->MovePoint(0,
								me->GetPositionX() + (cos(m_heading) * 10),
								me->GetPositionY() + (sin(m_heading) * 10),
								me->GetPositionZ());
						m_uiTimer = 5000;
						m_uiPhase = 3;
						break;
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		if (pPlayer->GetQuestStatus(QUEST_TROLL_PATROL_INTESTINAL_FORTITUDE)
				== QUEST_STATUS_INCOMPLETE)
	...
	bool OnGossipSelect(Player* pPlayer, Creature* pCreature,
		...
		if (uiAction == GOSSIP_ACTION_INFO_DEF + 1) {
			...
			CAST_AI(npc_crusade_recruit::npc_crusade_recruitAI, (pCreature->AI()))->m_uiPhase =
					1;
...
enum eScourgeEnclosure {
	QUEST_OUR_ONLY_HOPE = 12916, NPC_GYMER_DUMMY = 29928
//from quest template
};
...
public:
	...
	bool OnGossipHello(Player* pPlayer, GameObject* pGO) {
		if (pPlayer->GetQuestStatus(QUEST_OUR_ONLY_HOPE)
				== QUEST_STATUS_INCOMPLETE) {
			Creature* pGymerDummy = pGO->FindNearestCreature(NPC_GYMER_DUMMY,
					20.0f);
			if (pGymerDummy) {
				pGO->UseDoorOrButton();
				pPlayer->KilledMonsterCredit(pGymerDummy->GetEntry(),
						pGymerDummy->GetGUID());
