...
public:
	npc_alexstrasza_wr_gate() :
			CreatureScript("npc_alexstrasza_wr_gate") {
	}
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestRewardStatus(QUEST_RETURN_TO_AG_A)
				|| pPlayer->GetQuestRewardStatus(QUEST_RETURN_TO_AG_H))
...
public:
	npc_inquisitor_hallard() :
			CreatureScript("npc_inquisitor_hallard") {
	}

	struct npc_inquisitor_hallardAI: public npc_escortAI {
		npc_inquisitor_hallardAI(Creature* pCreature) :
	...
	bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
		...
		if (quest->GetQuestId() == QUEST_A_RIGHTEOUS_SERMON) {
			...
			if (npc_escortAI* pEscortAI = CAST_AI(npc_inquisitor_hallard::npc_inquisitor_hallardAI, pCreature->AI())) {
				...
				pCreature->GetMotionMaster()->MovePoint(0, 3801.543f, -679.350f,
						213.75f);
			}
		}
		return true;
