...
public:
	npc_injured_rainspeaker_oracle() :
			CreatureScript("npc_injured_rainspeaker_oracle") {
	}

	struct npc_injured_rainspeaker_oracleAI: public npc_escortAI {
		npc_injured_rainspeaker_oracleAI(Creature* c) :
				npc_escortAI(c) {
			c_guid = c->GetGUID();
		}
		...
		void WaypointReached(uint32 i) {
			...
			case 18:
				...
				me->AddUnitMovementFlag(
						MOVEMENTFLAG_SWIMMING | MOVEMENTFLAG_LEVITATING);
				break;
			case 19:
			...
			case 28:
				if (Player* pPlayer = GetPlayerForEscort())
					pPlayer->GroupEventHappens(
							QUEST_FORTUNATE_MISUNDERSTANDINGS, me);
				//  me->RestoreFaction();
		...
		void JustDied(Unit* /*killer*/) {
			...
			if (Player* pPlayer = GetPlayerForEscort()) {
				if (pPlayer->GetQuestStatus(QUEST_FORTUNATE_MISUNDERSTANDINGS)
...
enum eVekjik {
	GOSSIP_TEXTID_VEKJIK1 = 13137, GOSSIP_TEXTID_VEKJIK2 = 13138,
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_MAKING_PEACE)
				== QUEST_STATUS_INCOMPLETE) {
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
		...
		if (pPlayer->GetQuestStatus(QUEST_FREYA_PACT)
				== QUEST_STATUS_INCOMPLETE)
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AOF1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

		pPlayer->PlayerTalkClass->SendGossipMenu(GOSSIP_TEXTID_AVATAR1,
				pCreature->GetGUID());
		return true;
	}
...
public:
	...
	struct npc_bushwhackerAI: public ScriptedAI {
		npc_bushwhackerAI(Creature* pCreature) :
				...
				if (Unit* pSummoner = CAST_SUM(me)->GetSummoner())
					if (pSummoner)
						me->GetMotionMaster()->MovePoint(0,
								pSummoner->GetPositionX(),
								pSummoner->GetPositionY(),
...
public:
	...
	struct npc_engineer_heliceAI: public npc_escortAI {
		npc_engineer_heliceAI(Creature* pCreature) :
		...
		void WaypointReached(uint32 i) {
			...
			case 1:
				...
				me->CastSpell(5918.33f, 5372.91f, -98.770f,
						SPELL_EXPLODE_CRYSTAL, true);
				me->SummonGameObject(184743, 5918.33f, 5372.91f, -98.770f, 0, 0,
						0, 0, 0, TEMPSUMMON_MANUAL_DESPAWN); //approx 3 to 4 seconds
			...
			case 7:
				...
				break;
			case 8:
				me->CastSpell(5887.37f, 5379.39f, -91.289f,
						SPELL_EXPLODE_CRYSTAL, true);
				me->SummonGameObject(184743, 5887.37f, 5379.39f, -91.289f, 0, 0,
						0, 0, 0, TEMPSUMMON_MANUAL_DESPAWN); //approx 3 to 4 seconds
				me->HandleEmoteCommand(EMOTE_ONESHOT_LAUGH);
				break;
			case 9:
...
public:
	...
	struct npc_jungle_punch_targetAI: public ScriptedAI {
		npc_jungle_punch_targetAI(Creature* pCreature) :
				ScriptedAI(pCreature) {
		}
		...
		void SpellHit(Unit* caster, const SpellEntry* proto) {
			...
			QuestStatusMap::const_iterator itr =
					caster->ToPlayer()->getQuestStatusMap().find(QUEST_ENTRY);
...
public:
	...
	bool OnGossipHello(Player *pPlayer, Creature *pCreature) {
		...
		pPlayer->PlayerTalkClass->SendGossipMenu(GOSSIP_MENU_DWARF,
				pCreature->GetGUID());
		return true;
	}
