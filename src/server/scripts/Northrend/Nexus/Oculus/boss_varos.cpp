...
enum Says {
	SAY_AGGRO = 0, SAY_AZURE = 1, SAY_AZURE_EMOTE = 2, SAY_DEATH = 3
};
...
enum Events {
	EVENT_ENERGIZE_CORES = 1, EVENT_CALL_AZURE, EVENT_AMPLIFY_MAGIC
};
...
public:
	...
	struct boss_varosAI: public BossAI {
		boss_varosAI(Creature* creature) :
				BossAI(creature, DATA_VAROS_EVENT) {
		...
		void Reset() {
			...
			events.ScheduleEvent(EVENT_AMPLIFY_MAGIC,
					urand(20, 25) * IN_MILLISECONDS);
			events.ScheduleEvent(EVENT_ENERGIZE_CORES,
					urand(0, 15) * IN_MILLISECONDS);
			// not sure if this is handled by a timer or hp percentage
			events.ScheduleEvent(EVENT_CALL_AZURE,
					urand(15, 30) * IN_MILLISECONDS);
...
public:
	npc_azure_ring_captain() :
			CreatureScript("npc_azure_ring_captain") {
	}
...
public:
	spell_varos_centrifuge_shield() :
			SpellScriptLoader("spell_varos_centrifuge_shield") {
	}
	...
	class spell_varos_centrifuge_shield_AuraScript: public AuraScript {
		...
		void OnApply(AuraEffect const* /*aurEff*/,
			...
			if (GetCaster()->HasFlag(
					UNIT_FIELD_FLAGS,
					UNIT_FLAG_UNK_15 | UNIT_FLAG_PASSIVE
							| UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_UNK_6)) {
		...
		void Register() {
			OnEffectRemove +=
					AuraEffectRemoveFn(spell_varos_centrifuge_shield_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
			OnEffectApply +=
					AuraEffectApplyFn(spell_varos_centrifuge_shield_AuraScript::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
		}
	};
