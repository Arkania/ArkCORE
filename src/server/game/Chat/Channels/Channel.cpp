...
void Channel::Join (uint64 p, const char *pass)
{
    ...
    if (plr)
    {
        if (HasFlag(CHANNEL_FLAG_LFG) && sWorld->getBoolConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && plr->GetSession()->GetSecurity() == SEC_PLAYER && plr->GetGroup())
        {
            MakeNotInLfg(&data);
            SendToOne(&data, p);
            return;
...
void Channel::SetMode (uint64 p, const char *p2n, bool mod, bool set)
{
    ...
    else
    {
        ...
        if ((plr->GetSession()->GetSecurity() < SEC_GAMEMASTER || newp->GetSession()->GetSecurity() < SEC_GAMEMASTER) && plr->GetTeam() != newp->GetTeam() && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
        {
            WorldPacket data;
            MakePlayerNotFound(&data, p2n);
            SendToOne(&data, p);
...
void Channel::List (Player* player)
{
    ...
    else
    {
        ...
        for (PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
        {
            ...
            if (plr && (player->GetSession()->GetSecurity() > SEC_PLAYER || plr->GetSession()->GetSecurity() <= AccountTypes(gmLevelInWhoList)) && plr->IsVisibleGloballyFor(player))
            {
                data << uint64(i->first);
