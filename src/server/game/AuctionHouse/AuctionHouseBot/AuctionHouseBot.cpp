...
void AuctionHouseBot::addNewAuctions (Player *AHBplayer, AHBConfig *config)
{
    ...
    if (debug_Out)
        sLog->outString("AHSeller: Minimum Items %u", config->GetMinItems());
    if (debug_Out)
        sLog->outString("AHSeller: Maximum Items %u", config->GetMaxItems());
    ...
    {
        if (debug_Out)
            sLog->outString("AHSeller: Auctions disabled");
        return;
    }
    ...
    {
        if (debug_Out)
            sLog->outString("AHSeller: GetAuctionEntry error [%u]", config->GetAHFID());
        return;
    }
    ...
    {
        if (debug_Out)
            sLog->outString("AHSeller: GetAuctionsMap error [%u]", config->GetAHFID());
        return;
    }
    ...
    if (debug_Out)
        sLog->outString("AHSeller: Numbers of auctions [%u]", auctionHouse->Getcount());
    ...
    if (SellGlyphs)
    {
        ...
        {
            if (debug_Out)
                sLog->outString("AHSeller: Numbers of auctions is at MAX", (maxItems + GlyphsCount));
            return;
        }
    ...
    else
    {
        ...
        {
            if (debug_Out)
                sLog->outString("AHSeller: Auctions above minimum");
            return;
        }
        if (auctions >= maxItems)
        {
            if (debug_Out)
                sLog->outString("AHSeller: Auctions at or above maximum");
            return;
        }
    ...
    if (debug_Out)
        sLog->outError("config->GetAHID() [%u]", config->GetAHID());
    ...
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            if (debug_Out)
                sLog->outError("Lets see what the bill lands on [%u]", buyoutPrice * stackCount);
