...
bool BattlefieldTB::SetupBattlefield()
{
    ...
    m_enable                     = sWorld->getIntConfig(CONFIG_TOL_BARAD_ENABLE);
...
void BattlefieldTB::OnBattleEnd(bool endbytimer)
{
    ...
    if (!endbytimer)
    {
        SendWarningToAllInZone(BATTLEFIELD_TB_TEXT_WIN_KEEP,
            sObjectMgr->GetArkCoreStringForDBCLocale((GetDefenderTeam() == TEAM_ALLIANCE) ? BATTLEFIELD_TB_TEXT_ALLIANCE : BATTLEFIELD_TB_TEXT_HORDE));
    }
    else
    {
