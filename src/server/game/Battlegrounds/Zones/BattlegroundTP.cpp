...
enum BG_TP_Rewards
{
    BG_TP_WIN = 0, BG_TP_FLAG_CAP, BG_TP_MAP_COMPLETE, BG_TP_REWARD_NUM
};
...
void BattlegroundTP::EventPlayerClickedOnFlag (Player *Source, GameObject* target_obj)
{
    ...
    if (Source->GetTeam() == HORDE && this->GetFlagState(ALLIANCE) == BG_TP_FLAG_STATE_ON_BASE && this->m_BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
    ...
    if (Source->GetTeam() == ALLIANCE && this->GetFlagState(HORDE) == BG_TP_FLAG_STATE_ON_BASE && this->m_BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
