...
void SpellCastTargets::read(ByteBuffer & data, Unit * caster)
{
	...
	if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
	{
		...
		else data >> m_srcPos.PositionXYZStream();
	}
	else
	{
	...
	if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
	{
		...
		else data >> m_dstPos.PositionXYZStream();
	}
	else
	{
				...
				Caster), m_spellValue(new SpellValue(m_spellInfo))
{
	...
	else m_originalCasterGUID = m_caster->GetGUID();
			...
			&& !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CANT_REFLECTED))
	{
		for (int j = 0; j < MAX_SPELL_EFFECTS; ++j)
		{
			...
			if (!IsPositiveTarget(m_spellInfo->EffectImplicitTargetA [j],
					m_spellInfo->EffectImplicitTargetB [j])) m_canReflect =
					true;
			else m_canReflect =
...
void Spell::prepareDataForTriggerSystem(AuraEffect const * /*triggeredByAura*/)
{
	...
	switch (m_spellInfo->DmgClass)
	{
		case SPELL_DAMAGE_CLASS_MELEE:
			...
			else m_procAttacker |= PROC_FLAG_DONE_MAINHAND_ATTACK;
...
void Spell::AddUnitTarget(Unit* pVictim, uint32 effIndex)
{
	...
	else target.missCondition = SPELL_MISS_EVADE; //SPELL_MISS_NONE;
	...
	else target.reflectResult = SPELL_MISS_NONE;
...
void Spell::DoTriggersOnSpellHit(Unit *unit)
{
	...
	if (m_customAttr & SPELL_ATTR0_CU_LINK_HIT)
	{
		if (const std::vector<int32> *spell_triggered = sSpellMgr->GetSpellLinked(m_spellInfo->Id + SPELL_LINK_HIT)) for (std::vector <
			...
			else unit->CastSpell(unit, *i, true, 0, 0, m_caster->GetGUID());
	}
}
...
struct ChainHealingOrder : public std::binary_function <const Unit*,
		const Unit*, bool>
{
		...
		int32 ChainHealingHash(Unit const* Target) const
		{
							...
							MainTarget->ToPlayer()))
			{
				...
				else return 20000 - Target->GetMaxHealth() + Target->GetHealth();
			}
			else return 40000 - Target->GetMaxHealth() + Target->GetHealth();
		}
};
		...
		uint32 num, SpellTargets TargetType)
{
	...
	else SearchAreaTarget(tempUnitMap, max_range, PUSH_CHAIN, TargetType);
		...
		SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
	...
	else m_caster->GetMap()->VisitAll(pos->m_positionX, pos->m_positionY,
			radius, notifier);
	...
	switch (TargetType)
	{
		case SPELL_TARGETS_ENTRY:
		{
			...
			if (conditions.empty())
			{
				...
				else return SearchNearbyTarget(range, SPELL_TARGETS_ENEMY,
						effIndex);
			}
...
void Spell::SelectEffectTargets(uint32 i, uint32 cur)
{
	...
	switch (SpellTargetType [cur])
	{
		...
		case TARGET_TYPE_DEST_CASTER: //4+8+2
		{
			...
			else dist = GetSpellRadiusForFriend(
		...
		case TARGET_TYPE_DEST_TARGET: //2+8+2
		{
			...
			if (checkCollision) target->GetFirstCollisionPosition(pos, dist,
					angle);
			else target->GetNearPosition(pos, dist, angle);
					...
					switch (m_spellInfo->Id)
					{
						...
						default:
							...
							if (m_spellInfo->Effect [i]
									...
									unitList, radius, pushType,
									SPELL_TARGETS_ALLY);
							else SearchAreaTarget(unitList, radius, pushType,
									SPELL_TARGETS_ENEMY);
					}
				}
				break;
					...
					case 2812:  // Paladin Holy Wrath
						if (m_spellInfo->Effect [i] == SPELL_EFFECT_APPLY_AURA
								&& m_spellInfo->EffectApplyAuraName [i]
										== SPELL_AURA_MOD_STUN)
						{
							bool isElemsAndDragonkings = m_caster->HasAura(
									56420);    // Glyph of Holy Wrath
									...
									unitList.begin(); itr != unitList.end();)
							{
								...
								if ((CREATURE_TYPE_DEMON == typeCreature
										|| CREATURE_TYPE_UNDEAD == typeCreature)
										|| (isElemsAndDragonkings
		...
		AuraEffect const * triggeredByAura)
{
			...
			&& m_spellInfo->Targets & TARGET_FLAG_UNIT)
	{
		...
		if (m_caster->GetTypeId() == TYPEID_UNIT) target =
				m_caster->getVictim();
		else target = ObjectAccessor::GetUnit(*m_caster,
				m_caster->ToPlayer()->GetSelection());
	...
	if (m_customAttr & SPELL_ATTR0_CU_LINK_CAST)
	{
		if (const std::vector<int32> *spell_triggered = sSpellMgr->GetSpellLinked(m_spellInfo->Id))
		{
			for (std::vector <int32>::const_iterator i =
				...
				else m_caster->CastSpell(
						m_targets.getUnitTarget() ?
								m_targets.getUnitTarget() : m_caster, *i, true);
		}
	}
...
uint64 Spell::handle_delayed(uint64 t_offset)
{
	...
	for (std::list <GOTargetInfo>::iterator ighit =
...
void Spell::SendSpellStart()
{
	...
	else data.append(m_caster->GetPackGUID());
...
void Spell::TakePower()
{
	...
	else m_caster->ModifyPower(powerType, -m_powerCost / 5);
...
SpellCastResult Spell::CheckCast(bool strict)
{
	...
	if (target)
	{
		...
		if (non_caster_target)
		{
			...
			if (m_caster->GetTypeId() == TYPEID_PLAYER)
			{
				...
				if (m_spellInfo->Mechanic == MECHANIC_DISARM)
				{
					...
					else if (!target->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID)) return SPELL_FAILED_TARGET_NO_WEAPONS;
				}
			}
		...
		if (m_caster->GetTypeId() == TYPEID_PLAYER)
		{
			...
			if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN
					&& (633 == m_spellInfo->Id || 642 == m_spellInfo->Id
							|| 1022 == m_spellInfo->Id))
			{
			...
			&& !(m_spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_MOUNTED))
	{
		...
		else return SPELL_FAILED_NOT_MOUNTED;
	}
		...
		switch (m_spellInfo->Effect [i])
		{
			...
			case SPELL_EFFECT_SUMMON_PET:
			{
				if (m_caster->GetPetGUID()) //let warlock do a replacement summon
				{
					...
					else return SPELL_FAILED_ALREADY_HAVE_SUMMON;
				}
			...
			case SPELL_EFFECT_LEAP_BACK:
			{
						...
						&& m_caster->HasUnitState(UNIT_STAT_ROOT))
				{
					...
					else return SPELL_FAILED_DONT_REPORT;
				}
				break;
			}
		...
		switch (m_spellInfo->EffectApplyAuraName [i])
		{
			...
			case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
			{
						...
						&& m_originalCaster->isAlive())
				{
					...
					{
						if (pArea->flags & AREA_FLAG_NO_FLY_ZONE) return
								(_triggeredCastFlags
								...
								CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
						{
							OutdoorPvPWG *pvpWG =
									(OutdoorPvPWG*) sOutdoorPvPMgr->GetOutdoorPvPToZoneId(
											4197);
							if (m_originalCaster->GetZoneId() == 4197 && pvpWG
									&& pvpWG != 0 && pvpWG->isWarTime()) return
									(_triggeredCastFlags
											& TRIGGERED_DONT_REPORT_CAST_ERROR) ?
											SPELL_FAILED_DONT_REPORT :
											SPELL_FAILED_NOT_HERE;
...
CurrentSpellTypes Spell::GetCurrentContainer()
{
	...
	else return (CURRENT_GENERIC_SPELL);
}
...
{
	for (std::list <TargetInfo>::const_iterator itr =
			m_UniqueTargetInfo.begin(); itr != m_UniqueTargetInfo.end(); ++itr)
