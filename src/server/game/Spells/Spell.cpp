		...
		if (m_targetMask & TARGET_FLAG_ITEM)
			...
			if (m_itemTargetGUID == TRADE_SLOT_NONTRADED) // here it is not guid but slot. Also prevents hacking slots
				if (TradeData* pTrade = player->GetTradeData())
					m_itemTarget = pTrade->GetTraderData()->GetItem(
...
void SpellCastTargets::read(ByteBuffer & data, Unit * caster) {
	...
	if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION) {
		...
		if (m_srcTransGUID)
			m_srcTransOffset.Relocate(caster->GetTransOffsetX(),
	...
	if (m_targetMask & TARGET_FLAG_DEST_LOCATION) {
		...
		if (m_dstTransGUID)
			m_dstTransOffset.Relocate(caster->GetTransOffsetX(),
					caster->GetTransOffsetY(), caster->GetTransOffsetZ(),
					caster->GetTransOffsetO());
		else
...
Spell::Spell(Unit* Caster, SpellEntry const *info, bool triggered,
	...
	case SPELL_DAMAGE_CLASS_RANGED:
		m_attackType =
				IsRangedWeaponSpell(m_spellInfo) ? RANGED_ATTACK : BASE_ATTACK;
		break;
	default:
		// Wands
			...
			case SPELL_EFFECT_DUMMY: {
				...
				case 54044: // Carrion Feeder
				{
					...
					if (m_spellInfo->Id == 20577)
						result =
								FindCorpseUsing<Trinity::CannibalizeObjectCheck>();
					else
					...
					if (result) {
						...
						if (m_caster->GetTypeId() == TYPEID_PLAYER)
							m_caster->ToPlayer()->RemoveSpellCooldown(
									m_spellInfo->Id, true);
			...
			case SPELL_EFFECT_APPLY_AREA_AURA_PARTY:
				...
				if (m_spellInfo->Attributes
						& (SPELL_ATTR0_CASTABLE_WHILE_SITTING
								...
								| SPELL_ATTR0_UNK18 | SPELL_ATTR0_NOT_SHAPESHIFT)
...
void Spell::DoAllEffectOnTarget(TargetInfo *target) {
	...
	if (getState() == SPELL_STATE_DELAYED && !IsPositiveSpell(m_spellInfo->Id)
			&& (getMSTime() - target->timeDelay) <= unit->m_lastSanctuaryTime)
	...
	if (m_healing > 0) {
		...
		if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
			caster->ProcDamageAndSpell(unitTarget, procAttacker, procVictim,
					procEx, addhealth, m_attackType, m_spellInfo,
	...
	// Do damage and triggers
		...
		if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT) {
			...
			if (caster->GetTypeId() == TYPEID_PLAYER
				...
				caster->ToPlayer()->CastItemCombatSpell(unitTarget,
						m_attackType, procVictim, procEx);
		}
	...
	// Passive spell hits/misses or active spells only misses (only triggers)
		...
		// Do triggers for unit (reflect triggers passed on hit phase for correct drop charge)
		if (canEffectTrigger && missInfo != SPELL_MISS_REFLECT)
			caster->ProcDamageAndSpell(unit, procAttacker, procVictim, procEx,
					0, m_attackType, m_spellInfo, m_triggeredByAuraSpell);
		...
		if (spellHitTarget->GetTypeId() == TYPEID_UNIT) {
			...
			if (m_originalCaster && m_originalCaster->IsControlledByPlayer()
				...
				if (Player* p = m_originalCaster->GetCharmerOrOwnerPlayerOrPlayerItself())
					p->CastedCreatureOrGO(spellHitTarget->GetEntry(),
							spellHitTarget->GetGUID(), m_spellInfo->Id);
		}
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit *unit, const uint32 effectMask,
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
		if (effectMask & (1 << i)
				&& IsUnitOwnedAuraEffect(m_spellInfo->Effect[i]))
			aura_effmask |= 1 << i;
			...
			if (m_spellAura) {
				...
				if (IsChanneledSpell(m_spellInfo)) {
					if (m_spellInfo->AttributesEx5
						...
						m_originalCaster->ModSpellCastTime(aurSpellInfo,
								duration, this);
				}
...
bool Spell::UpdateChanneledTargetList() {
	...
	if (channelAuraMask) {
		...
		if (Player * modOwner = m_caster->GetSpellModOwner())
			modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_RANGE, range,
					this);
	}
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius,
	...
	if ((m_spellInfo->AttributesEx3 & SPELL_ATTR3_PLAYERS_ONLY)
		...
		m_caster->GetMap()->VisitWorld(pos->m_positionX, pos->m_positionY,
				radius, notifier);
	else
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType,
			...
			case SPELL_TARGET_TYPE_CONTROLLED:
				for (Unit::ControlList::iterator itr =
					...
					if ((*itr)->GetEntry() == (*i_spellST)->mConditionValue2
							&& (*itr)->IsWithinDistInMap(m_caster, range)) {
						goScriptTarget = NULL;
	...
	case SPELL_TARGETS_ENEMY: {
		Unit *target = NULL;
		Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster,
				range);
		Trinity::UnitLastSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(
	...
	case SPELL_TARGETS_ALLY: {
		Unit *target = NULL;
		Trinity::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster,
				range);
...
void Spell::SelectEffectTargets(uint32 i, uint32 cur) {
	...
	case TARGET_TYPE_DEST_DEST: //5+8+1
	{
		...
		if (cur == TARGET_DEST_DEST_RANDOM
				|| cur == TARGET_DEST_DEST_RANDOM_DIR_DIST)
	...
	case TARGET_TYPE_DEST_SPECIAL: {
		...
		case TARGET_DST_HOME:
			if (m_caster->GetTypeId() == TYPEID_PLAYER)
				m_targets.setDst(m_caster->ToPlayer()->m_homebindX,
	...
	case TARGET_TYPE_CHANNEL: {
		...
		case TARGET_DEST_CHANNEL_CASTER:
			m_targets.setDst(
					*m_originalCaster->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->GetCaster());
			break;
		}
		break;
	}
	...
	if (pushType == PUSH_CHAIN) // Chain
		...
		if (maxTargets > 1) {
			...
			case TARGET_UNIT_NEARBY_ENTRY: // fix me
				range = GetSpellMaxRange(m_spellInfo, false);
				if (modOwner)
					modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_RANGE,
							range, this);
			...
			if (!conditions.empty()) {
				for (ConditionList::const_iterator i_spellST =
					...
					if ((*i_spellST)->mConditionType
						...
						continue;
					if ((*i_spellST)->mConditionValue3
							&& !((*i_spellST)->mConditionValue3 & (1 << i)))
						continue;
				...
				case 51328:
					...
					if (!(m_targets.getUnitTarget()
							&& ((m_targets.getUnitTarget()->GetEntry() == 26125
		...
		default:
			...
			case TARGET_UNIT_AREA_PARTY_DST:
				...
				break;
			case TARGET_UNIT_PARTY_TARGET:
				m_targets.getUnitTarget()->GetPartyMemberInDist(unitList,
						radius);
				break;
			case TARGET_UNIT_PARTY_CASTER:
			...
			case TARGET_UNIT_CLASS_TARGET: {
				...
				if (pGroup) {
					for (GroupReference *itr = pGroup->GetFirstMember();
						...
						if (Target
								&& targetPlayer->IsWithinDistInMap(Target,
										radius)
			...
			if (uint32 maxTargets = m_spellValue->MaxAffectedTargets) {
				...
				case 71390: // Pact of the Darkfallen
				{
					...
					for (std::list<Unit*>::const_iterator itr =
							unitList.begin(); itr != unitList.end() && remove;
							++itr) {
				...
				if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT
					...
					// Pet not found - remove cooldown
						...
						if (modOwner->GetTypeId() == TYPEID_PLAYER)
							modOwner->RemoveSpellCooldown(m_spellInfo->Id,
									true);
...
void Spell::prepare(SpellCastTargets const* targets,
	...
	if (!m_targets.HasSrc()
			&& m_spellInfo->Targets & TARGET_FLAG_SOURCE_LOCATION)
		m_targets.setSrc(*m_caster);
	...
	if (m_IsTriggeredSpell
			&& (!IsChanneledSpell(m_spellInfo)
					|| !GetSpellMaxDuration(m_spellInfo)))
		cast(true);
	else {
		...
		for (std::list<TargetInfo>::const_iterator ihit =
			...
			if ((*ihit).missCondition == SPELL_MISS_NONE)
				if (Unit* unit = m_caster->GetGUID() == ihit->targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID))
					unit->RemoveOwnedAura(m_spellInfo->Id, m_originalCasterGUID,
...
void Spell::cast(bool skipCheck) {
	...
	if (m_spellInfo->SpellFamilyName) {
		if (m_spellInfo->excludeCasterAuraSpell
				&& !IsPositiveSpell(m_spellInfo->excludeCasterAuraSpell))
			m_preCastSpell = m_spellInfo->excludeCasterAuraSpell;
		else if (m_spellInfo->excludeTargetAuraSpell
				&& !IsPositiveSpell(m_spellInfo->excludeTargetAuraSpell))
...
void Spell::handle_immediate() {
	...
	if (IsChanneledSpell(m_spellInfo)) {
		...
		if (duration) {
			...
			// Apply duration mod
			if (Player* modOwner = m_caster->GetSpellModOwner())
				modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_DURATION,
						duration);
			// Apply haste mods
			...
			if (!IsAutoRepeat() && !IsNextMeleeSwingSpell()) {
				if (Player* p = m_caster->GetCharmerOrOwnerPlayerOrPlayerItself()) {
					for (std::list<TargetInfo>::iterator ihit =
						...
						p->CastedCreatureOrGO(unit->GetEntry(), unit->GetGUID(),
								m_spellInfo->Id);
					}
...
void Spell::TakePower() {
	...
	if (m_caster->GetTypeId() == TYPEID_PLAYER) {
		if (m_spellInfo->powerType == POWER_RAGE
				...
				|| m_spellInfo->powerType == POWER_RUNE
				|| m_spellInfo->powerType == POWER_RUNIC_POWER)
...
void Spell::TakeAmmo() {
	if (m_attackType == RANGED_ATTACK
		...
		if (!pItem || pItem->IsBroken()
				|| pItem->GetProto()->SubClass == ITEM_SUBCLASS_WEAPON_WAND)
			return;
...
void Spell::HandleHolyPower(Player* caster) {
	...
	if (m_spellInfo->powerType == POWER_HOLY_POWER) {
		if (uint64 targetGUID = m_targets.getUnitTargetGUID()) {
			...
			if (hit)
				modOwner->ApplySpellMod(m_spellInfo->Id, SPELLMOD_COST,
						m_powerCost);
...
SpellCastResult Spell::CheckCast(bool strict) {
	...
	if (!m_IsTriggeredSpell && !m_caster->isAlive()
			...
			&& !(m_spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD))
	...
	if (m_caster->GetTypeId() == TYPEID_PLAYER
		...
		if (m_spellInfo->Attributes & SPELL_ATTR0_OUTDOORS_ONLY
				&& !m_caster->GetMap()->IsOutdoors(m_caster->GetPositionX(),
						m_caster->GetPositionY(), m_caster->GetPositionZ()))
	...
	if (!m_IsTriggeredSpell) {
		...
		if (m_spellInfo->casterAuraSpell
			...
			return SPELL_FAILED_CASTER_AURASTATE;
		if (m_spellInfo->excludeCasterAuraSpell
				&& m_caster->HasAura(m_spellInfo->excludeCasterAuraSpell))
			return SPELL_FAILED_CASTER_AURASTATE;

		if (reqCombat && m_caster->isInCombat()
				&& IsNonCombatSpell(m_spellInfo))
	...
	if (target) {
		...
		if (m_spellInfo->targetAuraSpell
				&& !target->HasAura(m_spellInfo->targetAuraSpell))
			return SPELL_FAILED_TARGET_AURASTATE;

		if (m_spellInfo->excludeTargetAuraSpell
				&& target->HasAura(m_spellInfo->excludeTargetAuraSpell))
			return SPELL_FAILED_TARGET_AURASTATE;

		if (!m_IsTriggeredSpell && target == m_caster
				&& m_spellInfo->AttributesEx & SPELL_ATTR1_CANT_TARGET_SELF)
		...
		if (non_caster_target) {
			...
			if (target->HasUnitState(UNIT_STAT_IN_FLIGHT)
					|| (target->HasUnitState(UNIT_STAT_ONVEHICLE)
							&& target->GetVehicleBase() != m_caster))
				return SPELL_FAILED_BAD_TARGETS;
	...
	if (!m_IsTriggeredSpell
			&& (m_spellInfo->Effect[EFFECT_0] == SPELL_EFFECT_DISPEL))
		if (Unit* target = m_targets.getUnitTarget())
			if (!GetSpellRadius(m_spellInfo, EFFECT_0,
	...
	for (int i = 0; i < MAX_SPELL_EFFECTS; i++) {
		...
		case SPELL_EFFECT_POWER_DRAIN: {
			...
			if (m_caster->GetTypeId() == TYPEID_PLAYER)
				if (Unit* target = m_targets.getUnitTarget())
					if (target != m_caster
							&& target->getPowerType()
									!= Powers(m_spellInfo->EffectMiscValue[i]))
						return SPELL_FAILED_BAD_TARGETS;
			break;
		}
		case SPELL_EFFECT_CHARGE: {
		...
		case SPELL_EFFECT_OPEN_LOCK: {
			...
			// we need a go target in case of TARGET_GAMEOBJECT
					|| (m_spellInfo->EffectImplicitTargetA[i]
							== TARGET_GAMEOBJECT && !m_targets.getGOTarget()))
				return SPELL_FAILED_BAD_TARGETS;
			...
			SpellCastResult res = CanOpenLock(i, lockId, skillId, reqSkillValue,
					skillValue);
				...
				if (m_caster->GetTypeId() == TYPEID_PLAYER
					...
					if (strict) //starting cast, trigger pet stun (cast by pet so it doesn't attack player)
						if (Pet* pet = m_caster->ToPlayer()->GetPet())
							pet->CastSpell(pet, 32752, true, NULL, NULL,
									pet->GetGUID());
				} else
					return SPELL_FAILED_ALREADY_HAVE_SUMMON;
			}
		...
		case SPELL_EFFECT_LEAP_BACK: {
			...
			if (m_caster->GetTypeId() == TYPEID_PLAYER && m_spellInfo->Id == 781
					&& !m_caster->isInCombat())
	...
	for (int i = 0; i < MAX_SPELL_EFFECTS; i++) {
		...
		case SPELL_AURA_DUMMY: {
			...
			case 61336:
				if (m_caster->GetTypeId() != TYPEID_PLAYER
						|| !m_caster->ToPlayer()->IsInFeralForm())
			...
			case 1515: {
				...
				if (!target->GetCreatureInfo()->isTameable(
						m_caster->ToPlayer()->CanTameExoticPets()))
					return SPELL_FAILED_BAD_TARGETS;
		...
		case SPELL_AURA_MOUNTED: {
			...
			if (m_caster->GetTypeId() == TYPEID_PLAYER && !AllowMount
					&& !m_IsTriggeredSpell && !m_spellInfo->AreaGroupId)
				...
				if (AreaTableEntry const* pArea = GetAreaEntryByAreaID(m_originalCaster->GetAreaId()))
					if (pArea->flags & AREA_FLAG_NO_FLY_ZONE)
						return m_IsTriggeredSpell ?
								SPELL_FAILED_DONT_REPORT : SPELL_FAILED_NOT_HERE;
			}
			break;
		}
...
SpellCastResult Spell::CheckCasterAuras() const {
	...
	if (m_spellInfo->AttributesEx & SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY) {
		for (int i = 0; i < MAX_SPELL_EFFECTS; ++i) {
			if (m_spellInfo->EffectApplyAuraName[i]
				...
				dispel_immune |= GetDispellMask(
						DispelType(m_spellInfo->EffectMiscValue[i]));
		}
	...
	if (unitflag & UNIT_FLAG_STUNNED) {
		...
		if (usableInStun) {
			...
			&& !(m_spellInfo->AttributesEx5 & SPELL_ATTR5_USABLE_WHILE_CONFUSED))
		prevented_reason = SPELL_FAILED_CONFUSED;
	else if (unitflag & UNIT_FLAG_FLEEING
			&& !(m_spellInfo->AttributesEx5 & SPELL_ATTR5_USABLE_WHILE_FEARED))
...
SpellCastResult Spell::CheckRange(bool strict) {
	...
	if (m_targets.HasDst() && !m_targets.HasTraj()) {
		if (!m_caster->IsWithinDist3d(&m_targets.m_dstPos, max_range))
			return SPELL_FAILED_OUT_OF_RANGE;
		if (min_range
				&& m_caster->IsWithinDist3d(&m_targets.m_dstPos, min_range))
	...
	for (int i = 0; i < MAX_SPELL_EFFECTS; i++) {
		...
		case SPELL_EFFECT_CREATE_ITEM_2: {
			if (!m_IsTriggeredSpell && m_spellInfo->EffectItemType[i]) {
				...
				uint8 msg = p_caster->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
						m_spellInfo->EffectItemType[i], 1);
				if (msg != EQUIP_ERR_OK) {
		...
		case SPELL_EFFECT_MILLING: {
			...
			if (item_millingskilllevel
					> p_caster->GetSkillValue(SKILL_INSCRIPTION))
				return SPELL_FAILED_LOW_CASTLEVEL;
...
void Spell::DelayedChannel() {
	...
	for (std::list<TargetInfo>::const_iterator ihit =
		...
		if ((*ihit).missCondition == SPELL_MISS_NONE)
			if (Unit* unit = (m_caster->GetGUID() == ihit->targetGUID) ? m_caster : ObjectAccessor::GetUnit(*m_caster, ihit->targetGUID))
				unit->DelayOwnedAuras(m_spellInfo->Id, m_originalCasterGUID,
						delaytime);
...
bool Spell::CheckTargetCreatureType(Unit* target) const {
	...
	if (target->GetEntry() == 5925
			...
			&& m_spellInfo->EffectApplyAuraName[0] == SPELL_AURA_MOD_CONFUSE)
		return true;
...
bool Spell::IsValidSingleTargetSpell(Unit const* target) const {
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i) {
		if (!IsValidSingleTargetEffect(target,
				Targets(m_spellInfo->EffectImplicitTargetA[i])))
			return false;
		// Need to check B?
...
void Spell::CalculateDamageDoneForAllTargets() {
	...
	for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
		if (m_applyMultiplierMask & (1 << i))
			multiplier[i] = SpellMgr::CalculateSpellEffectDamageMultiplier(
					m_spellInfo, i, m_originalCaster, this);
...
void Spell::SetSpellValue(SpellValueMod mod, int32 value) {
	...
	case SPELLVALUE_BASE_POINT0:
		m_spellValue->EffectBasePoints[0] =
				SpellMgr::CalculateSpellEffectBaseAmount(value, m_spellInfo, 0);
		break;
	case SPELLVALUE_BASE_POINT1:
		m_spellValue->EffectBasePoints[1] =
				SpellMgr::CalculateSpellEffectBaseAmount(value, m_spellInfo, 1);
		break;
	case SPELLVALUE_BASE_POINT2:
		m_spellValue->EffectBasePoints[2] =
				SpellMgr::CalculateSpellEffectBaseAmount(value, m_spellInfo, 2);
		break;
