	...
	if (unitTarget && unitTarget->isAlive()) {
		...
		case SPELLFAMILY_GENERIC: {
			...
			case 92631: {
				...
				if (unitTarget->GetUnitMovementFlags() == MOVEMENTFLAG_JUMPING
						|| unitTarget->GetTypeId() != TYPEID_PLAYER)
					return;
				break;
			}
		...
		case SPELLFAMILY_WARLOCK: {
			...
			// Conflagrate - consumes Immolate
				...
				if (aura) {
					...
					uint32 pct_dir = m_caster->CalculateSpellDamage(unitTarget,
							m_spellInfo, (effIndex + 1));
					...
					uint32 pct_dot = m_caster->CalculateSpellDamage(unitTarget,
							m_spellInfo, (effIndex + 2)) / 3;
					m_spellValue->EffectBasePoints[1] =
							SpellMgr::CalculateSpellEffectBaseAmount(
									pdamage * baseTotalTicks * pct_dot / 100,
									m_spellInfo, 1);
		...
		case SPELLFAMILY_PRIEST: {
			...
			if ((m_spellInfo->SpellFamilyFlags[1] & 0x2)) {
				...
				if (back_damage < int32(unitTarget->GetHealth()))
					m_caster->CastCustomSpell(m_caster, 32409, &back_damage, 0,
							0, true);
			}
			...
			// Improved Mind Blast (Mind Blast in shadow form bonus)
				...
				for (Unit::AuraEffectList::const_iterator i =
						ImprMindBlast.begin(); i != ImprMindBlast.end(); ++i) {
					if ((*i)->GetSpellProto()->SpellFamilyName
							== SPELLFAMILY_PRIEST
		...
		case SPELLFAMILY_DRUID: {
			...
			if (m_caster->GetTypeId() == TYPEID_PLAYER
				...
				damage += int32(
						m_caster->ToPlayer()->GetComboPoints() * ap * 7 / 100);
			}
			// Starfire
			...
			// Wrath
				...
				if (AuraEffect const * aurEff = m_caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 1771, 0))
					if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
						...
						damage = int32(
								damage * (100.0f + aurEff->GetAmount())
										/ 100.0f);
		...
		case SPELLFAMILY_PALADIN: {
			...
			if (m_spellInfo->SpellFamilyFlags[1] & 0x00040000) {
				...
				float average = (m_caster->GetFloatValue(UNIT_FIELD_MINDAMAGE)
						+ m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE))
						/ float(m_caster->GetAttackTime(BASE_ATTACK) / 1000.0f);
		...
		if (m_originalCaster && damage > 0 && apply_direct_bonus)
			damage = m_originalCaster->SpellDamageBonus(unitTarget, m_spellInfo,
...
void Spell::EffectDummy(SpellEffIndex effIndex) {
	...
	case SPELLFAMILY_GENERIC: {
		...
		case 12868: {
			...
			if (m_caster->haveOffhandWeapon()
					&& m_caster->getAttackTimer(BASE_ATTACK)
							> m_caster->getAttackTimer(OFF_ATTACK))
				damage = (m_caster->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE)
						+ m_caster->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE))
						/ 2;
			else
				damage = (m_caster->GetFloatValue(UNIT_FIELD_MINDAMAGE)
						+ m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE)) / 2;
			...
			case 12868:
				...
				break; // Rank 3
			default:
				sLog->outError("Spell::EffectDummy: Spell %u not handled in DW",
						m_spellInfo->Id);
				return;
			};
			...
			AuraEffect* deepWound = unitTarget->GetAuraEffect(12721, 0,
					m_caster->GetGUID());
			if (deepWound) {
				int32 remainingTicks = deepWound->GetBase()->GetDuration()
		...
		case 13567: // Dummy Trigger
		{
			...
			case 26467: // Persistent Shield
				m_caster->CastCustomSpell(unitTarget, 26470, &damage, NULL,
						NULL, true);
				break;
			default:
				sLog->outError(
		...
		case 23019: // Crystal Prison Dummy DND
		{
			...
			m_caster->SummonGameObject(179644, creatureTarget->GetPositionX(),
					creatureTarget->GetPositionY(),
					creatureTarget->GetPositionZ(),
					creatureTarget->GetOrientation(), 0, 0, 0, 0,
					uint32(creatureTarget->GetRespawnTime() - time(NULL)));
			sLog->outDebug(
		...
		case 37674: // Chaos Blast
		{
			...
			m_caster->CastCustomSpell(unitTarget, 37675, &basepoints0, NULL,
					NULL, true);
			return;
		}
			...
			for (std::list<TargetInfo>::iterator ihit =
				...
				if (ihit->effectMask & (1 << effIndex)) {
					if (Unit* casttarget = Unit::GetUnit((*unitTarget), ihit->targetGUID))
						m_caster->DealDamage(casttarget, damage, NULL,
								SPELL_DIRECT_DAMAGE, SPELL_SCHOOL_MASK_ARCANE,
								spellInfo, false);
				}
		...
		case 43723: {
			...
			if (player
					&& player->GetQuestStatus(11379) == QUEST_STATUS_INCOMPLETE) {
				Creature *creature = player->FindNearestCreature(19973, 10,
						false);
				if (!creature) {
		...
		case 52759: // Ancestral Awakening
			if (!unitTarget)
				return;
			m_caster->CastCustomSpell(unitTarget, 52752, &damage, NULL, NULL,
					true);
			return;
					...
					if (Unit *passenger = vehicle->GetPassenger(damage - 1)) {
						...
						if (target
								&& target->GetBase()->IsWithinDist2d(
										&m_targets.m_dstPos,
										GetSpellRadius(m_spellInfo, effIndex,
												false) * 2)) // now we use *2 because the location of the seat is not correct
		...
		case 54577: // U.D.E.D.
		{
			...
			m_caster->SummonGameObject(192693, unitTarget->GetPositionX(),
					unitTarget->GetPositionY(), unitTarget->GetPositionZ(),
					unitTarget->GetOrientation(), 0, 0, 0, 0, 100);
			for (uint8 i = 0; i < 4; ++i)
				m_caster->SummonGameObject(191567,
						float(unitTarget->GetPositionX() + irand(-7, 7)),
						float(unitTarget->GetPositionY() + irand(-7, 7)),
		...
		case 51961: // Captured Chicken Cover - Quest 12702 & 12532
		{
			if (m_caster->GetTypeId() != TYPEID_PLAYER
					...
					|| !(m_caster->ToPlayer()->GetQuestStatus(12702)
							...
							|| m_caster->ToPlayer()->GetQuestStatus(12532)
									== QUEST_STATUS_INCOMPLETE))
				return;
		...
		case 79751: // Destroy Mechano-Tank
		{
			...
			m_caster->ToPlayer()->KilledMonsterCredit(unitTarget->GetEntry(),
					unitTarget->GetGUID());
		}
		...
		case 21562: // Power Word : Fortitude
		{
			if (m_caster->GetTypeId() == TYPEID_PLAYER) {
				...
				for (std::list<Unit*>::iterator itr = PartyMembers.begin();
						itr != PartyMembers.end(); ++itr) // If caster is in party with a player
						{
					++player;
	...
	case SPELLFAMILY_MAGE: {
		...
		if (m_spellInfo->SpellFamilyFlags[0]
			...
			if (m_caster->HasAura(11190)) // Improved Cone of Cold Rank 1
					{
				m_caster->CastCustomSpell(unitTarget, 83301, &bp, NULL, NULL,
						true, 0);
			}
			if (m_caster->HasAura(12489)) // Improved Cone of Cold Rank 2
					{
				m_caster->CastCustomSpell(unitTarget, 83302, &bp, NULL, NULL,
						true, 0);
			}
		}
		switch (m_spellInfo->Id) {
	...
	case SPELLFAMILY_WARRIOR:
		...
		if (m_spellInfo->SpellFamilyFlags & SPELLFAMILYFLAG_WARRIOR_CHARGE
			...
			m_caster->CastCustomSpell(m_caster, 34846, &chargeBasePoints0, NULL,
					NULL, true);
		...
		if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_WARRIOR_SLAM
			...
			int32 bp0 = damage;
			m_caster->CastCustomSpell(unitTarget, 50782, &bp0, NULL, NULL, true,
					0);
			return;
		}
		// Execute
		...
		// Bloodthirst
		case 23881: {
			m_caster->CastCustomSpell(unitTarget, 23885, &damage, NULL, NULL,
					true, NULL);
			return;
		}
			// Intercept
	...
	case SPELLFAMILY_SHAMAN:
		...
		if (m_spellInfo->SpellFamilyFlags[0]
			...
			if (unitTarget)
				m_caster->CastCustomSpell(unitTarget, 52025, NULL, &bp1, NULL,
		...
		if (m_spellInfo->SpellFamilyFlags[0]
			...
			m_caster->CastCustomSpell(unitTarget, 52042, &damage, 0, 0, true, 0,
		...
		if (m_spellInfo->SpellFamilyFlags[0]
			...
			if (!unitTarget || unitTarget->getPowerType() != POWER_MANA)
				return;
			m_caster->CastCustomSpell(unitTarget, 52032, &damage, 0, 0, true, 0,
					0, m_originalCasterGUID);
			return;
		}
		// Lava Lash
		if (m_spellInfo->SpellFamilyFlags[2]
			...
			if (m_caster->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0,
				...
				if (m_caster->GetAuraEffect(SPELL_AURA_DUMMY,
						SPELLFAMILY_SHAMAN, 0x200000, 0, 0))
	...
	case SPELLFAMILY_DEATHKNIGHT:
		...
		if (m_spellInfo->SpellFamilyFlags[0]
			...
			if ((m_caster->CountPctFromMaxHealth(7))
					> (20 * m_caster->GetDamageTakenInPastSecs(5) / 100))
			...
			if (AuraEffect const * aurEff = m_caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_DEATHKNIGHT, 2751, 0))
				bp = int32(
						bp
								* (m_caster->CalculateSpellDamage(m_caster,
										aurEff->GetSpellProto(), 2) + 100.0f)
								/ 100.0f);
		...
		if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_COIL) {
			if (m_caster->IsFriendlyTo(unitTarget)) {
				...
				m_caster->CastCustomSpell(unitTarget, 47633, &bp, NULL, NULL,
						true);
			} else {
				int32 bp = 985 + damage;
				m_caster->CastCustomSpell(unitTarget, 47632, &bp, NULL, NULL,
						true);
			}
			return;
		}
		...
		case 49560: // Death Grip
			...
			if (Unit *unit = unitTarget->GetVehicleBase()) // what is this for?
				...
				unitTarget->CastSpell(pos.GetPositionX(), pos.GetPositionY(),
						pos.GetPositionZ(), damage, true);
			return;
...
void Spell::EffectJump(SpellEffIndex effIndex) {
	...
	if (m_targets.getUnitTarget())
		m_targets.getUnitTarget()->GetContactPoint(m_caster, x, y, z,
				CONTACT_DISTANCE);
	else if (m_targets.getGOTarget())
		m_targets.getGOTarget()->GetContactPoint(m_caster, x, y, z,
				CONTACT_DISTANCE);
	else {
		sLog->outError(
	...
	if (m_targets.HasDst()) {
		...
		if (m_spellInfo->EffectImplicitTargetA[effIndex]
			...
			if (m_targets.getUnitTarget()
				...
				pTarget = ObjectAccessor::GetUnit(*m_caster,
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/) {
	...
	if (mapid == unitTarget->GetMapId())
		unitTarget->NearTeleportTo(x, y, z, orientation,
				unitTarget == m_caster);
...
void Spell::EffectPowerDrain(SpellEffIndex effIndex) {
	if (m_spellInfo->EffectMiscValue[effIndex] < 0
			|| m_spellInfo->EffectMiscValue[effIndex] >= int8(MAX_POWERS))
		return;
...
void Spell::EffectSendEvent(SpellEffIndex effIndex) {
	...
	if (unitTarget) {
		if (ZoneScript* zoneScript = unitTarget->GetZoneScript())
			zoneScript->ProcessEvent(unitTarget,
	...
	if (unitTarget && unitTarget->isAlive() && damage >= 0) {
		...
		// Swiftmend - consumes Regrowth or Rejuvenation
			...
			if (Unit* auraCaster = targetAura->GetCaster())
				tickheal = auraCaster->SpellHealingBonus(unitTarget,
						targetAura->GetSpellProto(), effIndex, tickheal, DOT);
		...
		// Death Pact - return pct of max health to caster
			...
			addhealth = caster->SpellHealingBonus(unitTarget, m_spellInfo,
					effIndex, int32(caster->CountPctFromMaxHealth(damage)),
					HEAL);
		else
			addhealth = caster->SpellHealingBonus(unitTarget, m_spellInfo,
					effIndex, addhealth, HEAL);
...
void Spell::EffectEnergize(SpellEffIndex effIndex) {
	...
	if (m_spellInfo->Id == 45051) {
		...
		for (Unit::AuraApplicationMap::iterator itr = Auras.begin();
			...
			if (!guardianFound)
				if (sSpellMgr->IsSpellMemberOfSpellGroup(spell_id,
	...
	if (gameObjTarget) {
		...
		case GAMEOBJECT_TYPE_BUTTON:
			...
			player->GetMap()->ScriptsStart(sGameObjectScripts,
					gameObjTarget->GetDBTableGUIDLow(), player, gameObjTarget);
			return;
...
WorldPacket			data(SMSG_TOTEM_CREATED, 1+8+4+4);
			...
			summon = m_caster->GetMap()->SummonCreature(entry, pos, properties,
					duration, m_originalCaster, 0, lowGUID);
	...
	if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK
		...
		if (Unit* pOwner = m_caster->GetOwner())
			if (pOwner->GetAura(56249))
				pOwner->CastCustomSpell(pOwner, 19658, &heal_amount, NULL, NULL,
						true);
	}
}
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/) {
	...
	if (unitTarget->isAlive()
		...
		m_caster->ToPlayer()->SendLoot(unitTarget->GetGUID(),
				LOOT_PICKPOCKETING);
}
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/) {
	...
	if (unitTarget->getThreatManager().getCurrentVictim()) {
		...
		if (itsThreat > myThreat)
			unitTarget->getThreatManager().addThreat(m_caster,
					itsThreat - myThreat);
	}
...
void Spell::SpellDamageWeaponDmg(SpellEffIndex effIndex) {
	...
	case SPELLFAMILY_HUNTER: {
		...
		case 63741: // Chimera Shot
		{
			...
			if (m_caster->HasAuraEffect(53241, 0, 0))
				if (roll_chance_i(
					...
					m_caster->CastSpell(m_caster->ToPlayer()->GetSelectedUnit(),
							88691, true);
					break;
				}
		...
		spell_bonus += int32(
				(shotMod * m_caster->GetTotalAttackPowerValue(RANGED_ATTACK)));
		break;
	}
	case SPELLFAMILY_DEATHKNIGHT: {
		// Plague Strike
		if (m_spellInfo->SpellFamilyFlags[EFFECT_0] & 0x1) {
			...
			if (AuraEffect const * aurEff = m_caster->GetAuraEffect(58657, EFFECT_0))
				totalDamagePercentMod *= (aurEff->GetAmount() + 100.0f)
						/ 100.0f;
			break;
		}
		// Blood Strike
...
void Spell::EffectHealMaxHealth(SpellEffIndex effIndex) {
	...
	if (m_originalCaster)
		m_healing += m_originalCaster->SpellHealingBonus(unitTarget,
				m_spellInfo, effIndex, addhealth, HEAL);
}
	...
	if (pGameObj->GetGoType() == GAMEOBJECT_TYPE_FLAGDROP
		...
		case 489: //WS
		{
			if (bg && bg->GetTypeID(true) == BATTLEGROUND_WS
				...
				((BattlegroundWS*) bg)->SetDroppedFlagGUID(pGameObj->GetGUID(),
						team);
			}
			break;
		}
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex) {
	...
	case SPELLFAMILY_GENERIC: {
			...
			if (AuraEffect const * aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00100000, 0, 0, m_caster->GetGUID())) {
				...
				if (countMin < countMax) {
					aurEff->GetBase()->SetDuration(
							uint32(aurEff->GetBase()->GetDuration() + 3000));
		...
		case 35376:
			if (unitTarget)
				unitTarget->ToPlayer()->TeleportTo(0, 1805.99f, 341.32f, 70.66f,
						1.6f);
			break;
		case 35727:
			if (unitTarget)
				unitTarget->ToPlayer()->TeleportTo(530, 10038.7f, -7000.9f,
						61.86f, 3.05f);
			break;
		case 45204: // Clone Me!
		...
		case 54426:
			if (unitTarget) {
				...
				if (damage > 0)
					m_caster->CastCustomSpell(28375, SPELLVALUE_BASE_POINT0,
							damage, unitTarget);
			}
			return;
		...
		case 48025: // Headless Horseman's Mount
		{
			...
			bool canFly = true;
			uint32 v_map = GetVirtualMapForMapAndZone(unitTarget->GetMapId(),
					unitTarget->GetZoneId());
			if (v_map != 530 && v_map != 571 && v_map != 0)
				canFly = false;

			if (canFly && v_map == 571
					&& !unitTarget->ToPlayer()->HasSpell(54197))
				canFly = false;

			if (canFly && v_map == 0
					&& !unitTarget->ToPlayer()->HasSpell(90267))
				canFly = false;
			...
			if (canFly && (pArea && pArea->flags & AREA_FLAG_NO_FLY_ZONE)
					|| !pArea)
				canFly = false;
		...
		case 47776: {
			...
			sprintf(
					...
					m_caster->GetName(), gender, urand(1, 6), urand(1, 6));
			m_caster->MonsterTextEmote(buf, 0);
			break;
		}
			// Vigilance
		...
		case 52479: // Gift of the Harvester
			if (unitTarget && m_originalCaster)
				m_originalCaster->CastSpell(unitTarget,
						urand(0, 1) ? damage : 52505, true);
			return;
			// Death Gate
		case 52751: {
		...
		case 58420: // Portal to Stormwind
		{
			...
			uint32 spellID = SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
					0);
			uint32 questID = SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
					1);
		...
		case 60123: // Lightwell
		{
			...
			if (chargesaura && chargesaura->GetCharges() > 1) {
				...
				unitTarget->CastSpell(unitTarget, spell_heal, true, NULL, NULL,
		...
		case 58591: // Rank 10
		{
			...
			for (uint8 slot = SUMMON_SLOT_TOTEM; slot < MAX_TOTEM_SLOT;
				...
				if (totem && totem->isTotem()) {
					m_caster->CastCustomSpell(totem, 55277, &basepoints0, NULL,
							NULL, true);
				}
			}
			...
			if (AuraEffect *aur=unitTarget->GetAuraEffect(63298, 0)) {
				...
				m_caster->CastCustomSpell(unitTarget, 55277, &basepoints0, NULL,
						NULL, true);
			}
			break;
		}
		case 66545: //Summon Memory
		{
			...
			uint32 uiSpells[26] = { 66704, 66705, 66706, 66707, 66709, 66710,
					66711, 66712, 66713, 66714, 66715, 66708, 66708, 66691,
					66692, 66694, 66695, 66696, 66697, 66698, 66699, 66700,
		...
		case 64142: // Upper Deck - Create Foam Sword
			...
			static uint32 const itemId[] = { 45061, 45176, 45177, 45178, 45179,
					0 };
	...
	case SPELLFAMILY_PALADIN: {
		...
		if (m_spellInfo->Category == SPELLCATEGORY_JUDGEMENT) {
			...
			for (Unit::AuraApplicationMap::iterator iter = sealAuras.begin();
					iter != sealAuras.end();) {
			...
			for (Unit::AuraApplicationMap::iterator iter = talentauras.begin();
					iter != talentauras.end();) {
				Aura * aura = iter->second->GetBase();
				switch (aura->GetId()) {
				case 31876: {
	...
	case SPELLFAMILY_HUNTER: {
		...
		if (m_spellInfo->SpellFamilyFlags[2] & 0x400000) {
			...
			if (unitTarget->GetAura(1978))
				unitTarget->GetAura(1978)->SetDuration(
		...
		if (m_spellInfo->Id == 85948) {
			...
			if (unitTarget->HasAura(45524)) // Chains of Ice
				...
				unitTarget->GetAura(45524)->SetDuration(
						(unitTarget->GetAura(45524)->GetDuration() + r * 1000),
						true);
			}
			if (unitTarget->HasAura(55095)) // Frost Fever
				...
				unitTarget->GetAura(55095)->SetDuration(
						(unitTarget->GetAura(55095)->GetDuration() + r * 1000),
						true);
			}
			if (unitTarget->HasAura(55078)) // Blood Plague
				...
				unitTarget->GetAura(55078)->SetDuration(
						(unitTarget->GetAura(55078)->GetDuration() + r * 1000),
						true);
			}
		}
		break;
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex) {
	if (m_caster->GetTypeId()
			!= TYPEID_PLAYER || m_glyphIndex >= MAX_GLYPH_SLOT_INDEX)
		return;
...
void Spell::EffectProspecting(SpellEffIndex /*effIndex*/) {
	...
	Player* p_caster = (Player*) m_caster;
	if (!itemTarget
			|| !(itemTarget->GetProto()->Flags & ITEM_PROTO_FLAG_PROSPECTABLE))
		return;
...
void Spell::EffectMilling(SpellEffIndex /*effIndex*/) {
	...
	Player* p_caster = (Player*) m_caster;
	if (!itemTarget
			|| !(itemTarget->GetProto()->Flags & ITEM_PROTO_FLAG_MILLABLE))
		return;
...
void Spell::EffectCreateTamedPet(SpellEffIndex effIndex) {
	if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER
			...
			|| unitTarget->getClass() != CLASS_HUNTER)
		return;
...
void Spell::EffectRedirectThreat(SpellEffIndex /*effIndex*/) {
	if (unitTarget)
		m_caster->SetReducedThreatPercent((uint32) damage,
				unitTarget->GetGUID());
}
...
void Spell::EffectRenamePet(SpellEffIndex /*effIndex*/) {
	if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT
			|| !unitTarget->ToCreature()->isPet()
			|| ((Pet*) unitTarget)->getPetType() != HUNTER_PET)
		return;
