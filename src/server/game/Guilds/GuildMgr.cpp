...
void GuildMgr::LoadGuilds()
{
    ...
    sLog->outString("Loading guilds definitions...");
    {
        ...
        //          0          1       2             3              4              5              6
        QueryResult result = CharacterDatabase.Query("SELECT guildid, name, leaderguid, EmblemStyle, EmblemColor, BorderStyle, BorderColor, "
        //    7                  8       9       10            11           12         13         14       15
                "BackgroundColor, info, motd, createdate, BankMoney, COUNT(gbt.guildid), xp, level, todayXP, XPCap  "
                "FROM guild g LEFT JOIN guild_bank_tab gbt ON guildid = gbt.guildid GROUP BY guildid ORDER BY guildid ASC");
