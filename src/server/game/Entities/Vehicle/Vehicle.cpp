...
Vehicle::Vehicle(Unit *unit, VehicleEntry const *vehInfo) :
	...
	case 160:
		...
		me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK_DEST,
				true);
	case 158:
		...
		me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_DECREASE_SPEED,
				true);
	...
	for (SeatMap::iterator itr = m_Seats.begin(); itr != m_Seats.end(); ++itr)
		if (Unit *passenger = itr->second.passenger) {
			...
			if (passenger->GetVehicle() != this)
				sLog->outCrash("Vehicle %u has invalid passenger %u.",
						me->GetEntry(), passenger->GetEntry());
			passenger->ExitVehicle();
			if (itr->second.passenger) {
				sLog->outCrash(
						"Vehicle %u cannot remove passenger %u. %u is still on vehicle.",
						me->GetEntry(), passenger->GetEntry(),
						itr->second.passenger->GetEntry());
...
bool Vehicle::AddPassenger(Unit *unit, int8 seatId, bool byAura) {
	...
	if (seatId < 0) // no specific seat requirement
		...
		for (seat = m_Seats.begin(); seat != m_Seats.end(); ++seat)
			if (!seat->second.passenger
					&& ((!byAura && seat->second.seatInfo->CanEnterOrExit())
							|| (byAura
									&& seat->second.seatInfo->IsUsableByAura())))
				break;
	...
	if (seat->second.seatInfo->m_flags
			&& !(seat->second.seatInfo->m_flags & VEHICLE_SEAT_FLAG_UNK11))
	...
	if (me->IsInWorld()) {
		...
		if (me->GetTypeId() == TYPEID_UNIT) {
			if (me->ToCreature()->IsAIEnabled)
				me->ToCreature()->AI()->PassengerBoarded(unit, seat->first,
						true);
