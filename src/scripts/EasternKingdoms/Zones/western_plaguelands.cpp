...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
        ...
        if (pPlayer->GetQuestRewardStatus(5237)
                || pPlayer->GetQuestRewardStatus(5238)) {
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature) {
        ...
        if (pPlayer->GetQuestStatus(QUEST_SUBTERFUGE) == QUEST_STATUS_COMPLETE
                ...
                && !pPlayer->HasAura(SPELL_SCARLET_ILLUSION)) {
...
public:
    ...
    struct npc_the_scourge_cauldronAI: public ScriptedAI {
        npc_the_scourge_cauldronAI(Creature *c) :
                ScriptedAI(c) {
        }
...
public:
    ...
    struct npc_andorhal_towerAI: public Scripted_NoMovementAI {
        npc_andorhal_towerAI(Creature *c) :
        ...
        void MoveInLineOfSight(Unit* pWho) {
            ...
            if (me->FindNearestGameObject(GO_BEACON_TORCH, 10.0f))
                CAST_PLR(pWho)->KilledMonsterCredit(me->GetEntry(),
                        me->GetGUID());
        }
    };
};
...
enum eTruuen {
    NPC_GHOST_UTHER = 17233, NPC_THEL_DANIS = 1854, NPC_GHOUL = 1791, //ambush
    ...
    SAY_WP_6 = -1800070
//Thank you my friend for making this possible. This is a day that I shall never forget! I think I will stay a while. Please return to High Priestess MacDonnell at the camp. I know that she'll be keenly interested to know of what has transpired here.
};

class npc_anchorite_truuen: public CreatureScript {
public:
    npc_anchorite_truuen() :
            CreatureScript("npc_anchorite_truuen") {
    }

    bool OnQuestAccept(Player* pPlayer, Creature* pCreature,
            Quest const *quest) {
        npc_escortAI* pEscortAI =
                CAST_AI(npc_anchorite_truuen::npc_anchorite_truuenAI, pCreature->AI());
    ...
    struct npc_anchorite_truuenAI: public npc_escortAI {
        npc_anchorite_truuenAI(Creature* pCreature) :
        ...
        void WaypointReached(uint32 i) {
            ...
            case 8:
                ...
                me->SummonCreature(NPC_GHOUL, me->GetPositionX() + 7.0f,
                        me->GetPositionY() + 7.0f, me->GetPositionZ(), 0.0f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000);
                me->SummonCreature(NPC_GHOUL, me->GetPositionX() + 5.0f,
                        me->GetPositionY() + 5.0f, me->GetPositionZ(), 0.0f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000);
                break;
            case 9:
            ...
            case 14:
                ...
                me->SummonCreature(NPC_GHOUL, me->GetPositionX() + 5.0f,
                        me->GetPositionY() + 5.0f, me->GetPositionZ(), 0.0f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000);
                me->SummonCreature(NPC_GHOUL, me->GetPositionX() + 10.0f,
                        me->GetPositionY() + 10.0f, me->GetPositionZ(), 0.0f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000);
                me->SummonCreature(NPC_GHOUL, me->GetPositionX() + 8.0f,
                        me->GetPositionY() + 8.0f, me->GetPositionZ(), 0.0f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000);
                break;
            case 15:
                DoScriptText(SAY_WP_2, me);
            case 21:
                Theldanis = GetClosestCreatureWithEntry(me, NPC_THEL_DANIS,
                        150);
            ...
            case 22:
                break;
            case 23:
                Ughost = me->SummonCreature(NPC_GHOST_UTHER, 971.86f, -1825.42f,
                        81.99f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                        30000);
